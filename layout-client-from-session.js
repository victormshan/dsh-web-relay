  ...
  factory: (require) => {
    ...
    // ---- 样式 ----
      ...
      const DOCK_CSS = '' +
        ...
        '.dwr-rail{position:fixed;top:60;right:0;bottom:0;width:28px;display:flex;align-items:center;justify-content:center;background:var(--dsw-alias-bg-overlay,#fff);border-left:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.12));cursor:pointer;z-index:2147483000}'
    ...
    const inputStyle = {
      ...
      background: 'var(--dsw-alias-bg-base, #fff)', color: 'var(--dsw-alias-label-primary, #111)', border: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.15))',
    ...
    const btnGhostStyle = { ...btnStyle, background: 'var(--dsw-alias-bg-layer-2, #f0f0f0)', color: 'var(--dsw-alias-label-primary, #111)' }
    const btnWarnStyle = { ...btnStyle, background: 'var(--dsw-alias-state-error-primary, #dc2626)', color: '#fff' }
    const hintStyle = { color: 'var(--dsw-alias-label-secondary, #888)', fontSize: 12, margin: '4px 0' }
    const warnStyle = { color: 'var(--dsw-alias-state-warn-primary, #d97706)', fontSize: 12, margin: '4px 0' }
    ...
    const preStyle = {
      background: '#18181b', border: '1px solid #3f3f46', borderRadius: 6,
        background: 'var(--dsw-alias-bg-layer-1, #fafafa)', border: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.15))', borderRadius: 8,
    ...
    const tabBtnStyle = {
      ...
      background: 'transparent', color: 'var(--dsw-alias-label-secondary, #666)', cursor: 'pointer', fontSize: 12
    }
    ...
    const RelayPanel = (props) => {
      ...
      // v0.5: docked right sidebar + splitter (replaces floating drag)
      ...
      const [splitterDragging, setSplitterDragging] = useState(false)
        ...
        // Legacy floating-drag stubs kept for compatibility with old code paths.
        const dragRef = useRef(null)
        const setPos = () => {}
        const pos = { top: null, left: null }
      ...
      const copyHandoff = async (text) => {
        try {
          ...
          setError('剪贴板不可用，请手动复制下方文本')
            ...
            // ---- v0.5 Step 2: docked sidebar body padding + splitter drag ----
      useEffect(() => {
        if (!open || collapsed) {
      ...
      return h('div', {
        style: {
          ...
          // docked panel: body padding is driven by data-dwr-docked on <html>
        ...
        // onMouseLeave removed in docked mode
      },
          ...
          h('div', {
            style: {
              ...
              padding: '10px 12px', borderBottom: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.08))',
        ...
        h('div', { style: panelBodyStyle },
            ...
            actions.map((a) => h('div', {
              ...
              style: {
                ...
                background: '#18181b', border: '1px solid #3f3f46',
                  background: 'var(--dsw-alias-bg-layer-1, #fafafa)', border: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.15))',
          ...
          execResults && execResults.length > 0 && h('div', { style: { marginTop: 8 } },
            ...
            execResults.map((r) => h('div', {
              key: r.index,
              style: { background: 'var(--dsw-alias-bg-layer-1, #fafafa)', border: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.15))', borderRadius: 8, padding: 6, marginTop: 6 }
        ...
        h('div', { style: panelBodyStyle },
          ...
          traces === null
            ...
            : traces.length === 0
              ...
              : traces.map((t) => h('div', { key: t.id, style: { background: 'var(--dsw-alias-bg-layer-1, #fafafa)', border: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.15))', borderRadius: 8, padding: 8, marginBottom: 8 } },
    ...
    const apply = (ctx) => {
        // Inject dock styles once. Kept separate from the component so the
        // body padding and splitter hover styles are available immediately.
        const style = document.createElement('style')
        style.setAttribute('data-plugin-css', 'dsh-web-relay')
        style.textContent = DOCK_CSS
        document.head.append(style)
        ctx.effect(() => () => style.remove(), 'dsh-web-relay styles')
