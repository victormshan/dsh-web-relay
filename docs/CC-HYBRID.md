...
### 19.3 行号引用启发式（WARN 级）的四条实测修正

首版连续踩四个坑，均由**自测 / 反向验证**抓出（记录以备勿重犯）：
...
3. **对并发编辑误报**：审计正在执行的另一任务规格时报「漂移」，实为该任务**正在编辑目标文件**。已在告警文案中
   ...
   才说明行号确实写错。
4. **路径片段被误选为探针**（最隐蔽）：平局（出现次数相同）时排序稳定，会选中**行内先出现**的 token——而先出现的
   往往是**文件路径自身的片段**。实测 `lib/cc-channel.js 约 L650 export function shouldSkipForKnownQuotaExhaustion`：
   `channel`（路径片段）与真符号出现次数**都是 1**，于是探针选了 `channel`，窗口内找不到 → **对正确的行号误报**。
   已修正为：排除来自路径的 token，且**平局时取更长者**（更长 ≈ 更专有）。
