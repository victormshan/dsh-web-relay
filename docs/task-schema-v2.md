...
> 版本：s2v1（AutoIteration expr-2026-09-09_12-30-09 V1/6）
...
node scripts/task-schema-cli.mjs validate-result <taskDir>     # done.flag 根 + result.json + expectArtifacts
...
- `test/task-schema-v2.test.mjs`：29 用例（fake fs/exec，跨平台 path.join 修复）——validateTask 合法/非法/容错、validateResultText、validateResult（done.flag 根/out misplaced/result 结构/expectArtifacts/自定义 outputDir）、runAcceptanceScript
- 全量基线：269+（独立仓）
...
| 场景 | v1 | v2 |
|---|---|---|
| done.flag 放 out/ | runner 误判 failed（事故）| validateResult 报 misplaced 硬拦截 |
| result.json 结构 | status/reason 自由 | status 枚举 + exit + failed-reason 校验 |
| 期望产物 | 无 | expectArtifacts 存在性硬校验 |
| 验收 | Claude 自判 | acceptanceScript 可选执行钩子 |
