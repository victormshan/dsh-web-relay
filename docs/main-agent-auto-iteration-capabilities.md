...
| E12 | 宿主重启续跑（cross-restart resume，v4.0） | lib/resume-scan.js（isExprInterrupted/resumeAction/resumeHandoff 纯函数）+ lib/index.js：CURRENT_BOOT_ID 每进程唯一 + writeStepState 打戳/read 白名单（sessionId/bootId/restartCount）；ask/execute 创建落盘 sessionId + record frontmatter；apply 尾 bootResumeScan（跨 bootId 忙态 expr → restartCount+1：resume→trace+wakeMainAgent 自动唤醒 / 无 sessionId 降级留痕；≥2 熔断 paused 防死循环；rejectStreak/iterationBaseCommit 跨重启保持）；/admin/resume-scan 手动端点；health-check 暴露 bootId | ✅ 实现（v4.0.0 8f9cc13，expr-2026-09-05_13-35-19 external approved）｜E2E：端点扫描 fixture→restartCount 0→1→trace [system] 留痕→幂等；熔断 fixture restartCount 1→paused；候选基准修复（DSH_RELAY_WORKSPACE 注入 launcher）；**v4.4.1 系列**：wake 成功打标 resumeQueuedAt、isExprInterrupted 排除 paused/stopped（fe7f1ab）、真实会话续跑完整测试 3 连通过（hello+三时间戳实证） | resume-scan.test 4 例；docs/OPS-RESTART-RESUME.md §2；全量 167/167 |
| E13 | 完全自主三方协作 AutoIteration 实证（v4.1-4.3） | 以插件自身做 iterations=3 自主迭代：版间门自动推进（V1→V2→V3 currentIteration 自动 +1 + handoff trace）、11 步全 external approved 零打回、逐版 bump+tag+push（v4.1.0/v4.2.0/v4.3.0）、三轮零 lib 重启；产出面板托管状态呈现（bootId/续跑徽标）、运维 SOP（OPS-RESTART-RESUME.md）、审计日志、效果分析（AUTO-ITERATION-ANALYSIS.md §〇-§七 含重启账目与复盘修正） | ✅ 实现并实证（expr-2026-09-05_13-58-07，8e3b09a）｜观察摩擦：声明叙述式未解析（lesson 031）、.test.js 命名陷阱（lesson 030）、重启编排命令中断→手动兜底（lesson 032） | docs/AUTO-ITERATION-ANALYSIS.md；client-restart.test 2 例；全量 154/154 |
