...
|---|---|---|---|---|
| E1 | 影子试错沙盒（Shadow Trial Sandbox） | lib/index.js L841-915（/shadow create/merge/destroy/list；git worktree add --detach L874 + git diff/apply 原子合并 L888-890 + remove --force L892/905；SHADOW_MAX=2/5s） | ✅ 实现（v3.0.0 f9822b3）｜⚠️ **无业务调用方**（裸端点，仅测试与即席编排使用；AutoIteration 未接线） | expr-08-28_01-29-26；外部 AI 01-57-18"超预期"评价（worktree 复用 275ms/2.1MB、diff+apply 防污染、destroy 零污染断言）；shadow.test.js 真实闭环 |
