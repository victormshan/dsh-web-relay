...
## 10. 持续自动迭代的支撑条件总览（2026-09-04 主 agent 框架整理，用户定调）

> 用户定调：**热路径加载（hot path / hot reload）与影子沙盒共同构成"版本持续自动迭代"的支撑条件**——
> "改 → 验 → 生效 → 下一版"循环要能连续、少人值守地转起来，需要三类支撑协同：

| 支撑类别 | 机制 | 现状 | 摩擦点/演进 |
|---|---|---|---|
| **门禁与节律** | 审核链（external→dialog→manual）、版间门、熔断、restructure、突破度门禁(P3) | ✅ v1.9-v3.5.0 已齐 | 审核需外部 AI/人工在场 → 降级链兜底 |
| **安全试错** | 影子沙盒 L1（内存/Staging 语法预检，<50ms）+ L2（worktree 隔离+GC+原子应用/回滚） | 🟡 v3.0 端点 + v3.6.0 Step1-4 实施中 | 本设计文档主项 |
| **低摩擦交付（热路径）** | client 侧刷新即生效（HMR/静态重载）；host 侧改动**聚合到一次重启**收口；未来探索"重启编排/热重载" | 🟡 client=刷新即生效；host=需重启 | 历史探路：v1.0.0 时代曾做"热加载可行性探路"（expr-2026-08-25_02-09-32 内 Step）；dsh cordis loader 树对新增/启用行非热重载，故 host 采用"批量变更+单次重启收口"（v3.6.0 即如此：一次重启验证 Step2-4） |

**协同原则（写进实施纪律）**：
1. **能热就不重启**：client/pure-lib 改动走热路径（刷新）；新增依赖/路由/宿主生命周期改动才安排重启。
2. **重启要成批**：host 改动攒批（如一个版本的一次重启验证多步），减少人工介入次数——热路径把"等待重启"从每步降为每版本。
3. **热与影子分工**：影子管"改错可回滚"，热路径管"改对可快速生效"；二者共同支撑 v1.9 版间门连续推进。
4. 未来候选（记录愿景，非本期）：宿主热重载/重启编排（watchdog 式）作为 v3.7+ 支撑增强。
