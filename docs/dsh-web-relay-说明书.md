...
| 文件 | 作用 |
|---|---|
| `package.json` | 插件元数据，version 1.5.2 |
...
### 4.17 面板 Step List 可见性与自动载入修复（v1.5.1 / v1.5.2）

#### 1. 问题现象

- 多版本自动迭代（AutoIteration）执行期间，面板 Step List 无法自动载入最新任务状态：本次 3 版迭代主任务（expr-2026-08-27_15-42-26）全程 12 步 approved，但用户反复刷新面板，看到的始终是**旧任务**（expr-2026-08-27_15-26-14，done）。
- 重启后（v1.5.1 排序修复生效），自动载入又**误中版间门评审辅助 Ask**（expr-2026-08-27_15-52-11）：界面显示其参考 Step List（Step 9-12 全 pending ×2 重复），看起来"Step 9 以后都没开始"，与主任务实际状态（全 approved）完全不符。

#### 2. 根因复盘

- **后端截断 + 文件名排序（v1.5.1）**：
  - `/context` 端点硬编码 `records.slice(0, 3)` 只返回前 3 条记录；
  - 记录按**文件名字典序**而非创建时间排序——主任务记录 `dsh-web-relay-2026-08-27_15-42-26.md` 排第 5（前面压着 `v1.0.0-test-report`、验证方案 `validation.md`、两个评审 ask），**被 slice 截掉**；
  - 面板 `exprId` 为空时自动载入 `stepStates[0]` → 拿到旧任务（done），用户看到的状态与执行进度脱节。
- **评审 Ask 污染最新任务（v1.5.2）**：
  - 版间门唤醒主 agent 请外部 AI 评审上版产出时调用 `/ask`，系统为辅助评审生成**独立 exprId + steps.json**；
  - v1.5.1 改为按 `created` 降序后，评审 ask（created 更晚）排到最前，面板自动载入误将辅助评审的参考 Step List 当成主执行任务。

#### 3. 修复方案与演进

- **v1.5.1（commit `8dc877f`，历史列表扩容 + 排序修正）**：
  - 后端：按 `created` 时间戳降序（最新任务在前），返回上限 3 → 20 条；
  - 前端：Step List 载入卡片新增**实验选择下拉**（context 记录列表，选中即载入对应 exprId），面板打开自动拉取记录列表。
- **v1.5.2（commit `7f6c106`，辅助 Ask 状态隔离，治本）**：
  - `/ask` 新增 `auxiliary: true` 标记；评审等辅助请求**不再创建 stepState**（评审 Step List 仅供主 agent restructure 参考，不是要执行的状态机）；
  - 从根上杜绝辅助记录进入"最新任务"自动载入路径。

#### 4. 教训

- 协议数据的"最新任务"语义应由**创建时间 + 主/辅任务分类**共同决定，不能依赖文件名前缀（`dsh-web-relay-*` / `expr-*` / `validation.md` 会打乱时间序）。
- 面板"自动载入"是用户的默认入口，任何会把辅助数据推到这个位置的改动（排序、截断、前缀约定）都要回归验证——v1.5.1 的排序修复即引入了 v1.5.2 要修的评审 ask 污染场景。

### 4.18 v2.0.0 真实增量五迭代（expr-2026-08-27_20-20-48）

> 用户发起"再升级 5 个迭代"（expr-2026-08-27_20-20-48）。外部 AI 首版规划 4 步是 v1.9 已实现功能的重造（AutoIteration 熔断/降级链/DAG 门控/混合模式免审——代码全部已存在），主 agent 核查后止损，重新规划为 5 个**真实增量**方向并经外部 AI 确认，全部实施 + 外部审核通过。

| 迭代 | 增量 | 实现位置 | 关键实测 |
|---|---|---|---|
| v2.0 | 面板 Version Gate 进度条 + 熔断状态卡片（paused 高亮 + rejectStreak + Resume） | `lib/client.js` | 双语字典 + 复用 resumeExperiment |
| v2.1 | 面板 Step List DAG 拓扑图（分层 flex + 层间箭头 + 状态色，点击展开） | `lib/client.js` | dagLayers 纯函数 + 状态色映射 |
| v2.2 | auto-review 上一步产物摘要注入（截断 2000/文件，降低盲审打回） | `lib/index.js` | buildArtifactsSummary + buildReviewPrompt 扩展 |
| v2.3 | web-gemini 扩展多 Tab 负载均衡（最久未用优先分发 + 异常 Tab 摘除） | `dsh-web-gemini-ext/background.js` | tabActivity + pickIdleTab |
| v2.4 | 三端 Health Checker（/health-check 端点 + 面板状态灯 + 实时版本） | `lib/index.js` + `lib/client.js` | PLUGIN_VERSION 修复 statusHandler 硬编码 1.3.0 漂移 |

**回归与发布**：全量测试 44/44 通过（7 个测试文件）；package.json 1.5.2 → **2.0.0**；git tag v2.0.0 push victormshan/DSH。

**经验**：外部 AI 对"升级"类任务的规划需主 agent 核查——它可能因上下文缺失把**已有功能当新需求重造**；主 agent 用"代码位置证据"止损并重新定向（restructure），是三方协作中执行判断力的关键环节。

### 4.19 v2.2.0 运维自愈双迭代（expr-2026-08-27_22-15-02）

> 用户发起"再升级 2 个迭代"（expr-2026-08-27_22-15-02）。外部 AI 规划方向正确（真实增量，无重造），聚焦**调试研发效能 + 系统告警自愈**，全部实施 + 外部/dialog 审核通过。

| 迭代 | 增量 | 实现位置 | 关键实测 |
|---|---|---|---|
| v2.5 | **Trace Replay 离线沙盒**：`POST /dsh-web-relay/replay` 按 exprId+stepId 离线还原步骤 timeline（notes 状态流转序列）、最终审核记录（lastRev）、相关三方发言（start 时间起 20 条）——纯本地读取，不发起外部 AI | `lib/index.js` | replayHandler + offline:true |
| v2.6 | **Webhook 通知中心**：`DSH_RELAY_WEBHOOK_URL` 配置 + `notifyWebhook` fire-and-forget（失败静默）；熔断（circuit_breaker）/降级（degradation）/审核打回（review_rejected）三触发点；面板通知中心（URL 配置保存/测试发送/桌面 Notification 权限） | `lib/index.js` + `lib/client.js` | 熔断状态变化触发桌面 Notification（prevPausedRef 去重） |

**回归与发布**：全量测试 56/56 通过（9 个测试文件）；package.json 2.1.1 → **2.2.0**；git tag v2.2.0 push victormshan/DSH。

**用法**：面板通知中心行输入 Webhook URL → 保存 → 测试；后端也可用 `DSH_RELAY_WEBHOOK_URL` 环境变量（优先）。熔断 paused 时浏览器桌面通知自动弹出（需先授权）。

### 4.20 v3.0.0 影子试错沙盒范式突破（expr-2026-08-28_01-29-26）

> 突破类型：**Paradigm**（从"原位修改"到"无感知影子沙盒隔离试错"）。由外部 AI 突破设计师（Architect Pioneer）提案（突破度三阶评估 + 反模式预警 + 5 段式探路契约），主 agent 完成只读探路回传 5 段式数据（v3.0-shadow-probe.md）后实施。

**范式转变**：

| 维度 | 旧范式（≤v2.6） | V3.0 影子沙盒 |
|---|---|---|
| 执行环境 | 主 agent 直接修改主工作区 | git worktree **影子层**隔离试错 |
| 验证与审核 | 修改后直接回写，打回再修 | 影子层内自检 → diff 打包 → 审核 |
| 状态回滚 | 打回依赖手动撤销（v1.8.1 不自动回滚） | 打回**秒级销毁影子层**，主工作区零污染 |
| 合并机制 | 逐步无隔离落地 | **approved 后** diff 原子应用回主工作区 |

**实现（lib/index.js）**：
- `POST /dsh-web-relay/shadow` 四动作：**create**（`git worktree add --detach` 建影子层，复用对象库，实测 ~275ms/2.1MB）、**merge**（影子层内 `diff baseCommit` → `git apply` 原子应用回主工作区 → remove）、**destroy**（`worktree remove --force` 秒级清理）、**list**
- 约束：并发影子层 ≤2（磁盘 3.1GB 硬约束实测）、git 命令超时 5s
- 测试：`test/shadow.test.js` 含 **git worktree 创建-修改-merge-destroy 真实集成闭环**（临时仓库验证零污染 + diff 应用）

**关键实测**：git 2.54 worktree 支持；DSH 仓库检出 2.1MB（复用对象库，不复制 node_modules）；Windows 路径引号处理。
...
## 11. 影子沙盒与三方协作驱动的持续迭代架构（v3.6.0）

> 生成：2026-09-05（v3.6.0 三方协作复盘；外部 AI 评审 expr-2026-09-05_00-40-25）

### 11.1 三方协作闭环
- 用户定调（方向/验收/最终拍板）→ 外部 AI（方案规划 + 按证据审核）→ 主 agent（执行/收口/打回重提）。
- 证据型 complete：每步附执行日志/测试数/commit，证据不足即被外部 AI 打回（v3.6.0 5 步审核、2 次补证闭环实证）。

### 11.2 双层影子门禁（S8/S9）
- L1：内存/Staging 语法预检（node --check，<50ms，不建 worktree）；L2：git worktree 隔离 + node_modules 挂载 + GC + 原子应用/回滚（非 git 降级增量回滚）。
- complete 钩子：失败拒绝 complete 并回传 blocked 错误清单；面板 Disable Shadow 全局开关。

### 11.3 热路径与持续迭代支撑（三类协同）
- 门禁节律（审核链/版间门/熔断/突破度）+ 安全试错（影子 L1/L2）+ 低摩擦交付（热路径：client 刷新即生效；host 改动成批单次重启）。
- 详见 docs/auto-iteration-shadow-support-design.md §10。
