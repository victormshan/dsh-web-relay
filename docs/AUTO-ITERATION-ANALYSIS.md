...
> 时间：2026-09-05 · 三轮实验零用户参与；**但实验前的 v4.0 工程/E2E 阶段宿主重启多次、其中数次需用户手动兜底**（见 §七 重启账目与复盘修正）

## 〇、范围澄清（诚实边界）

「零用户参与」仅对 **v4.1→v4.3 三轮 AutoIteration** 成立（三轮纯 client/docs/test，零 lib 重启）。
v4.0 重启续跑机制的**开发与 E2E 阶段**（本实验前）发生多次宿主重启：
- **watchdog 自主重启 ~6 次**（日志实证：当前 watchdog 日志 4 次拉起 + 早期日志 2 次；miss×3→prepare→拉起，~40s 自愈，无需人）。
- **用户手动重启 ≥3 次**（宿主 PID 18008/18164 等 PPID=人工 launcher 而非 watchdog）：根因是**主 agent 的多步宿主操作编排命令多次被运行时中断**（watchdog 已被 kill 而 Start-ScheduledTask/树杀未执行 → 半状态），用户兜底手动启动。
- 教训：**触发链可靠 ≠ 触发动作可靠**——watchdog 一旦被触发能自愈，但「由 agent 工具调用编排重启」在多步命令中易被中断；缓解：单步幂等小命令、树杀作为回合最后动作、watchdog 单例锁 + 首检保证手动兜底也安全（见 lesson 032）。
...
| 阶段 | 重启类型 | 次数 | 说明 |
|---|---|---|---|
| 三轮实验（v4.1-4.3） | 宿主重启 | **0** | 纯 client/docs/test，无 lib 改动 |
| v4.0 开发/E2E（实验前） | watchdog 自主 | ~6 | 日志实证：当前 watchdog 日志 4 + 早期日志 2；miss×3→prepare→拉起，~40s 无感自愈 |
| v4.0 开发/E2E（实验前） | **用户手动兜底** | ≥3 | 编排命令被运行时中断→半状态（host 18008/18164 PPID=人工 launcher）；用户手动启动 |

**复盘修正**：
1. 「触发链可靠 ≠ 触发动作可靠」——watchdog 一旦触发必自愈；但由 agent 工具调用**编排**重启（多步：kill watchdog→Start-ScheduledTask→树杀 host）多次在命令中途被中断，留下半状态，最终靠用户手动兜底。这正是 v4.0「重启续跑」要消解的摩擦——**当前只消除了「重启后任务状态丢失」，未消除「重启动作本身对用户的手动依赖」**。
...
- git：7b04a6b(v4.1.0)、c1bbb1b(v4.2.0)、8e3b09a(v4.3.0)
- watchdog 日志：C:\Users\Administrator\.dsh\logs\dsh-web-watchdog.log（4 次拉起）+ _analysis/v39-watchdog-real*.log（2 次）
- 会话证据：主 agent 每步 complete 注记（证据文本）+ 用户「已重启/人为启动」消息时间线
