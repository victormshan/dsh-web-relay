...
- **证据必须"属于该对象"，否则宁可 SKIP 不判**（2026-09-21 实测）：`verify-cc-task` 的「写入范围」原本拿
  **实时工作树**判定任何任务，于是**主 agent 的施工窗口**（改文件未提交的那几分钟）会被判成"某个历史任务越界"——
  一次周期审计因此误报 ACTION-NEEDED。修正后的语义：
  · **在飞任务** → 用**实时工作树**（那才是它的产物；守卫不变）；
  · **已结算任务 + 有归属提交**（提交信息含 taskId）→ 用**提交归属**；
  · **已结算任务 + 无归属提交**（例：`cc-marker-missing` 的失败任务从未提交）→ **`[SKIP] 不适用`**，
    **绝不退回实时工作树**（那份改动属于**当前写入者**）。
  **SKIP 的语义**：打印但不计入 ACCEPT/REJECT（`verify-cc-task` 的 `skip()`），即"本次没有验证它的写入范围"——
  **不是 PASS**，也不是 FAIL。教训：判据要选**归属正确的证据**；证据归属错了，"判定"只是在测量别人。
...
- **已机械化（②，2026-09-20）**：`deliver-three-copies.mjs` 与 `restart-with-checklist.mjs` 都支持
  **`--guard <name>`** —— 检查在任何落盘/登记/重启请求**之前**执行：无记录 → **4 未验证**（不当成"没变"）、
  已变动 → **1 拒绝**（列出变化），两者都**不会**继续交付/重启。dry-run 也照样检查（提前告警）。
  接线由门禁 `verify-guard-wiring.mjs`（7 例）守：含"无记录→4 且**不发生交付**（无汇总行）"与
  "已变动→1 且**不发生交付**"两条负控（证明拒绝真的在落盘之前），以及对重启工具的顺序断言
  （**不真跑**——那会请求一次重启；触发型工具只做静态接线断言）。

### 8.11 重启清单登记**必须**先检查主槽：被占时拒绝，而不是静默入队
- **事故（2026-09-21 实测，我造成）**：主槽被探针的 `dryRun` 占位夹具堵住时，`restart-with-checklist.mjs`
  把清单**入队**了；而 boot 扫描与心跳**都只读主槽** ⇒ 没人消费它 ⇒ **重启成功执行、唤醒却从未发生**。
  更糟的是堵住主槽的正是**我自己**的验证探针：`verify-autoir-events.mjs` 的"队列收敛"用例每轮（审计 30 分钟 +
  每次回归）往真实通道漏一条 `dryRun` 夹具；`dryRun` 按设计**永不唤醒也永不销账** ⇒ 永久占位 ⇒
  通道静默失效数小时（清点：主槽循环销账 42 条、队列残留 19 条），期间真实告警全在排队。
- **机制性修复（两条，都带门禁）**：
  ① `restart-with-checklist.mjs` 登记前检查主槽：被**别的来源的未销账**信号占用 → **拒绝并报错（exit 1）**，
     明确写出"清单只会入队 → 重启后零唤醒"；确需入队须显式 `--queue-ok`（逃生口）。
     门禁 `verify-signal-write-guard.mjs` G5 三条守：有检查、有逃生口、**检查在写入之前**（顺序断言）。
  ② 那条用例改为**完全无副作用**：临时文件验证读方容忍两种路径/两种形态 + **静态写方契约断言**
     （插件必须写规范路径 + 裸数组），不再占用真实主槽（13 例，含"4 条→3 个不同 id"的去重断言）。
- **通用教训（值得推广）**：**验证共享状态的用例，绝不能用"占用共享状态"的办法制造条件** ——
  它跑在周期门禁里，泄漏会累积；更隐蔽的是它会**靠自己漏出的东西满足自己的前提**（自维持的"通过"）。
  正解：临时文件 + 静态契约断言，零副作用。
