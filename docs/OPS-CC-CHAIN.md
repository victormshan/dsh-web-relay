...
- **重启后必跑工具链回归**（一条命令，确认 relay 路由契约未漂移、守卫与断点语义未变）：
  ```powershell
  node "D:\dsh relay test\regression-post-restart.mjs"      # 期望 RESULT: 14/14 通过（前置条件：仓库工作树干净）
  ```
  覆盖：体检 / 验收器（已提交=ACCEPT、不存在任务=前置错误 exit 2）/ 收口器（按真实状态：已批准=noop、
  未批准=拒绝 exit 3）/ V1 验收与版间门（按真实状态：完成态=PASS、未完成=BLOCKED）/ 声明器探测
  （端点已落地=通过、未落地=exit 4）/ 规格模板扫描 / 链条探针模式（无副作用）/ 规格门控 /
  断点清理自测 / 额度解析自测 / 协议门自测 / 审核映射自测。
  **设计要点（2026-09-15 修复假失败）**：套件先读权威状态（`steps.json` + `/steps/declare` 探测）再**推导**期望，
...
- **验收器的提交归属窗口**：`verify-cc-task.mjs` 在工作树干净时按任务 id 在**最近 N 条提交**里找归属
  ...
  归属判定退化为「无产物」→ 一个早已 ACCEPT 的任务被判 REJECT。窗口不该是随时间流逝而失效的隐式契约。
