...
const GATES = [
  ...
  // v4.12.2（2026-09-23）: 手机链路的**端到端**实测（鉴权语义 + SPA 外壳 + 静态资源字节一致性）。
  // current-link.mjs 只证明 token 有效，不证明手机打开能加载；两者都要在门禁里。
  { name: '手机链路判据自检（正控 + 负控）', script: 'verify-mobile-link.mjs', args: ['--selftest'] },
