...
export function apply(ctx) {
  ...
  const traceHandler = async (req, res) => {
    ...
    const stepUpdateHandler = async (req, res) => {
      try {
        ...
        if (action === 'start') {
          ...
          // v1.7 (P5): 前置校验——缺 artifacts 仅警告，不阻止状态流转
    ...
    async function wakeAfterApproved(updated, step, reviewerLabel, sessionId, base, safePolicy) {
      ...
      const ready = updated.protocolVersion === 'v1.6' ? readySteps(updated) : []
      if (ready.length > 0) {
        // v1.6：就绪步骤 ≥ 1 → 并发清单唤醒
        nextSteps = ready
        nextStep = ready[0]
        ...
        if (batchStepIds.length > 0) {
          ...
          for (const sid of batchStepIds) {
            ...
            if (out.result === 'approved') {
              await wakeAfterApproved(out.updated, s, out.reviewerLabel, sessionId, base, safePolicy)
            } else if (out.result === 'rejected') {
              await wakeAfterRejected(exprId, s, out.reason, sessionId, base, safePolicy)
            }
          }
          const updated = await writeStepState(base, exprId, state, safePolicy).catch(() => state)
          return json(res, 200, { ok: true, batch: true, batchResults, stepState: updated })
        }
