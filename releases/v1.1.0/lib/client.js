    ...
    const RelayPanel = (props) => {
      ...
      useEffect(() => {
        ...
        fetch('/dsh-web-relay/protocol')
          ...
          .then((d) => { if (!cancelled && d && d.ok) setProtocol({ version: d.version, text: d.text, v16: d.protocolV16 || null, v17: d.protocolV17 || null, en: d.en || null }) })
      ...
      const packContext = async () => {
        ...
        try {
          ...
          const pickProtocol = (v15, v16, v17) => {
          ...
          // v1.7: 优先取 protocolV17 全文，其次 v1.6 的 protocolV16，再退回 v1.5 protocol
          ...
          const lines = [
            ...
            protocolVersion === 'v1.7' ? T.protoV17Directive : protocolVersion === 'v1.6' ? T.protoV16Directive : T.protoV15Directive,
            ...protocolText.split('\n'),
            // v1.7 T5: 5 段式打包模板（仅 v1.7 注入：data_schema / pricing_map / mount_points / runtime_limits / history_trace）
        ...
        h('div', { style: panelBodyStyle },
          ...
          protocol && h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, margin: '2px 0' } },
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
              ...
              }, (showProtocol ? T.protocolCollapse : T.protocolExpand) + ' ' + (protocolVersion === 'v1.6' && protocol.v16 ? 'v1.6' : protocolVersion === 'v1.7' && protocol.v17 ? 'v1.7' : 'v1.5'))
            ),
            showProtocol && h('div', { style: { ...preStyle, marginTop: 0, maxHeight: 160, fontSize: 12, width: '100%', boxSizing: 'border-box' } },
              (() => {
                ...
                const pick = protocolVersion === 'v1.6' && active.protocolV16 ? active.protocolV16
                ...
                return h('div', { key: s.id, style: { marginTop: 6, padding: 6, background: 'var(--dsw-alias-bg-layer-1, #18181b)', border: '1px solid var(--dsw-alias-border-l1, #3f3f46)', borderRadius: 6 } },
                  h('div', { style: { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' } },
                    ...
                    s.reviewedBy && h('span', {
                      style: {
                        fontSize: 10, fontWeight: 600, marginLeft: 6, padding: '1px 6px', borderRadius: 4, whiteSpace: 'nowrap',
                        color: s.reviewedBy === 'external' ? '#93c5fd' : s.reviewedBy === 'dialog' ? '#a78bfa' : '#fbbf24',
                        border: '1px solid ' + (s.reviewedBy === 'external' ? 'rgba(147,197,253,.5)' : s.reviewedBy === 'dialog' ? 'rgba(167,139,250,.5)' : 'rgba(251,191,36,.5)')
                      }
                    }, s.reviewedBy === 'external' ? T.reviewerExternal : s.reviewedBy === 'dialog' ? T.reviewerDialog : T.reviewerManual),
