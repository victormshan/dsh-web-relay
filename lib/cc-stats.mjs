...
const FAILURE_RULES = [
  ...
  [
    'runner-failed',
    /claude\s*exit\s*(≠|!=)\s*0|exit\s*(≠|!=)\s*0|done\.flag\s*(missing|不存在|缺失)|v2-validate-failed|runner[-_ ]?failed|\bcc-failed\b/i,
  ],
 ...
 *   - status==='done' → 计入 ok；
 *   - errorCode==='cc-marker-missing' → 单独计入 markerMissing，**不计入 failed**、
 *     也不进入 byFailure（该分类下产物完好只是没写完成标记，多次独立验收 ACCEPT，
 *     混进 failed 会误导 /health-check 的降级判断）；
 *   - 其余（含 status==='failed' 与 status 缺失/异常等一切无法判定为 done 的情况）
 *     → 计入 failed，并用既有 classifyCcFailure(errorCode || reason || '') 归类到
 *     byFailure（无法归类时 classifyCcFailure 本身兜底为 'unknown'，不抛错）；
...
export function summarizeChainTasks(results, { now = Date.now(), windowMs = CHAIN_WINDOW_MS_DEFAULT } = {}) {
  ...
  for (const item of list) {
    ...
    if (status === 'done') {
      ok += 1;
    } else if (isMarkerMissing) {
      markerMissing += 1;
      category = 'cc-marker-missing';
    } else {
      failed += 1;
      const reasonText = errorCode || (typeof item.reason === 'string' ? item.reason : '');
