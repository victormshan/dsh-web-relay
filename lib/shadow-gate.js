...
// v3.9 Step1（repoPath 自持回退）: 本模块自身所在目录（lib/）——不依赖任何 env 也能定位仓库根。
...
// ---------- repoPath 识别 ----------
// v4.11.0 步8：仅"是 git 根"不足以证明候选就是 dsh-web-relay 插件仓库——宿主工作区自身常年也是
// git 仓库（工具链纳入版本控制后），候选顺序里第一个只做 git 根判定的候选会抢先短路，
// 后面更精确的候选（如 moduleDir 从 lib/ 上溯出的插件自身仓库根）永远走不到（实测故障：
// /steps/update 的影子门禁在宿主工作区里找 lib/client.js，Cannot find module）。
// 修复：候选除了必须是 git 根，还必须在根目录下有 package.json 且 name === 'dsh-web-relay'；
// 不满足特征 → 返回 null（调用方 resolveShadowRepo 的候选循环见 null 即 continue 试下一个，
// 本函数自身不做"直接短路"式判断，短路与否由调用方循环语义保证）。
export function resolveRepoPath(base) {
  if (!base) return null
  let root
  try {
    root = execSync(`git -C ${q(base)} rev-parse --show-toplevel`, { encoding: 'utf8', timeout: 3000, stdio: ['ignore', 'pipe', 'pipe'] }).trim()
  } catch (err) {
  ...
  try {
    ...
    return (pkg && pkg.name === 'dsh-web-relay') ? root : null
  } catch (err) {
    return null // 无 package.json 或不可解析 → 不具备插件仓库特征
  }
}

// ---------- v3.9 Step1: 影子仓库根自持解析（不依赖 env，env 仅作可选加速项）----------
// 背景：base（宿主工作区）常年不是 git 仓库；env 名称跨版本不一致（DSH_RELAY_REPO_PATH vs
// DSH_RELAY_REPO）；watchdog 拉起宿主时继承的是自己启动时的旧环境，补 env 不可靠。
// 兜底思路：本模块自身（lib/shadow-gate.js）就躺在仓库里，用 import.meta.url 反推 moduleDir，
// 对 moduleDir 做 git 根解析即可拿到仓库根——全程不读任何环境变量也能工作。
//
// shadowRepoCandidates：仅做候选构造（原始值，不做 git 校验），便于纯函数单测优先级顺序。
export function shadowRepoCandidates({ base, payload, env, moduleDir } = {}) {
  const dir = moduleDir || MODULE_DIR
  return [
    base || null,
    (payload && typeof payload.repoPath === 'string' && payload.repoPath) || null,
    (env && typeof env.DSH_RELAY_REPO_PATH === 'string' && env.DSH_RELAY_REPO_PATH) || null,
    (env && typeof env.DSH_RELAY_REPO === 'string' && env.DSH_RELAY_REPO) || null,
    dir || null
  ]
}

// resolveShadowRepo：按候选顺序，返回第一个经 resolveRepoPath 确认为 git 根的路径；全不可用 → null。
export function resolveShadowRepo({ base, payload, env, moduleDir } = {}) {
  const candidates = shadowRepoCandidates({ base, payload, env, moduleDir })
  for (const c of candidates) {
    if (!c) continue
    const root = resolveRepoPath(c)
    if (root) return root
  }
  return null
}
...
// v3.7.0 P2: 取当前 HEAD（回滚基线记录用）；非 git 返回 null
export function getGitHead(repoPath) {
  if (!repoPath) return null
  try {
    return git(repoPath, 'rev-parse HEAD').trim() || null
  } catch (err) {
    return null
  }
}

// v3.8 Step2（GC 定时化）: 周期解析纯函数 —— 0/负/非法 → 0（禁用定时，仅保留 finalize 触发）；默认 600000ms；上限 24h
export function gcScheduleMs(value) {
