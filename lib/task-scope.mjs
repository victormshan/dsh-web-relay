...
// 2026-09-10 二次校准（dogfooding 发现假阳性）：编号项与深度动作词**设计数上限**——
// prompt 中用于"描述要求/枚举验收点"的编号列表（如测试用例清单 1)2)3)...13)）并不等于
// 逐项深度评估工作量，实测：stab1-4 任务（13 个编号验收点 + 3 深度词）实际 4-5 分钟成功，
// 初版却估 770s→too-big。上限取值使"编号多但无实质深度"的任务回落到 risky/ok，
// 同时保持 RCA 型（3 产物 + 多深度动作）仍判 too-big。
export const MAX_EVAL_ITEMS_COUNTED = 4
export const MAX_DEEP_ACTIONS_COUNTED = 4
...
export function assessTaskScope(task = {}) {
  ...
  // 二次校准：编号项/深度词按上限计（见常量注释——描述性编号列表不等于评估工作量）
  const evalCounted = Math.min(evalItemCount, MAX_EVAL_ITEMS_COUNTED)
  const deepCounted = Math.min(deepActionCount, MAX_DEEP_ACTIONS_COUNTED)
  ...
  const evalSeconds = evalCounted * SECONDS_PER_EVAL_ITEM
  const refsSeconds = refsChunks * SECONDS_PER_REFS_CHUNK
  const deepSeconds = deepCounted * SECONDS_PER_DEEP_ACTION
  ...
  if (evalItemCount > 0) reasons.push(`显式枚举评估项 ${evalItemCount}${evalItemCount > evalCounted ? `（计前 ${evalCounted} 项）` : ''}（+${evalSeconds}s）`)
  if (refsChunks > 0) reasons.push(`refs ${refsCount} 个，每 ${REFS_CHUNK_SIZE} 个 +${SECONDS_PER_REFS_CHUNK}s（+${refsSeconds}s）`)
  if (deepActionCount > 0) reasons.push(`深度动作 ${deepActionCount} 项${deepActionCount > deepCounted ? `（计前 ${deepCounted} 项）` : ''}（核实/评估/补丁/实现类，+${deepSeconds}s）`)
