...
/** 仲裁结果类型。 */
export const CROSS_CHECK_STATES = Object.freeze([
  'consensus-approved',
  'consensus-rejected',
  'escalate-conflict',            // 两通道结论冲突 → 必须人工
  'escalate-unusable-primary',    // 主通道不可用（无可采信结论）→ 必须人工
  'degrade-unusable-secondary',   // 仅次通道不可用（主通道结论可用）→ 采信主通道但标注未交叉校验
])
...
export function crossCheckVerdicts(primary, secondary, opts = {}) {
  ...
  // 主通道不可用 → 没有可采信的结论，必须人工（不得由次通道单方定论）
  if (!p.usable) {
    return {
      state: 'escalate-unusable-primary',
      result: null,
      escalate: true,
      reason: `交叉校验不可用：主通道（${pl}）未给出可识别结论（${p.reason.slice(0, 200)}）——不得由单方结论自动采信，升级人工`,
      detail,
    }
  }

  // 仅次通道不可用 → 主通道结论可用：降级采信但显式标注"未交叉校验"（区分"通道故障"与"结论冲突"，
  // 避免把通道故障误升级为人工负担）。escalate=false，调用方应按单通道采信 + 标注。
  if (!s.usable) {
    return {
      state: 'degrade-unusable-secondary',
      result: p.verdict,
      escalate: false,
      degraded: true,
      reason: `交叉校验未完成：次通道（${sl}）未给出可识别结论（${s.reason.slice(0, 200)}）——采信主通道（${pl}=${p.verdict}）但标注「未经交叉校验」\n【${pl}】${p.reason.slice(0, 400)}`,
      detail,
    }
  }
