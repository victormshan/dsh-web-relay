...
/**
 * 解析配额恢复时间时的默认时区（2026-09-16 新增）。
 * 真实文案通常自带 `(Asia/Shanghai)`；当文案省略时区时不至于整体解析失败——本环境的
 * claude 配额文案即使用该时区，故以此兜底（可被文案内的显式时区覆盖）。
 */
 ...
 * 挂钟分量，**以「目标挂钟分量」为基准**做差修正猜测，2-3 次迭代即收敛。
 *
 * 2026-09-16 实测修正（**8 小时时区偏差**）：原实现写的是 `diff = guess - seenMs`——
 * 以「当前猜测」而非「目标挂钟分量」做差，于是每次迭代固定偏一个时区偏移量（Asia/Shanghai 为 -8h），
 * 3 次迭代后偏 **-24h**；随后 parseResetsAt 的「目标已过 → 顺延一天」补偿又把它推回 +8h 的位置，
 * 净效果是**解析结果整体晚 8 小时**。实测：`resets 7am (Asia/Shanghai)` 解析为
 * `2026-09-16T07:00:00Z`（本地 15:00），正确应为 `2026-09-15T23:00:00Z`（本地 07:00）。
 * 影响：`shouldSkipForKnownQuotaExhaustion` 的「不盲等」短路会**按错误的恢复时刻**执行
 * （本例多跳 8 小时）。既有用例只断言「解析出的是未来时间」，故从未暴露该偏差（已补精确时刻断言）。
 * 抛出异常（如非法 timeZone）时由调用方 catch。
 */
function zonedTimeToUtcMs(y, mo, d, h, mi, timeZone) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const diff = target - seenMs; // 修正点：以目标挂钟分量为基准
    if (diff === 0) break;
    guess += diff;
  }
  return guess;
 ...
 *
 * 2026-09-16 实测修正（**分钟可选 + 时区可选**）：本环境 claude 的真实文案是
 * `You've hit your session limit · resets 7am (Asia/Shanghai)`——**只有小时、没有分钟**，
 * 而原正则要求 `H:MM` 且强制时区括号 → 恒解析为 null → `shouldSkipForKnownQuotaExhaustion`
 * 拿不到 resetsAt → 已设计的「已知耗尽不盲等」短路**不生效**（会继续白耗派发尝试）。
 * 实测证据：2026-09-16T18:57Z 的任务失败日志全文 60 字节，正是上述「仅小时」形态。
 * 现放宽为：分钟可省略（默认 0）；时区可省略（回退默认 Asia/Shanghai，见 DEFAULT_QUOTA_TZ）。
...
export function parseResetsAt(text, { now } = {}) {
  ...
  // 分钟缺省视为 0（真实文案「resets 7am」即此形态）
  const minute = mmRaw === undefined ? 0 : parseInt(mmRaw, 10);
  if (!Number.isFinite(hour) || !Number.isFinite(minute)) return null;
  // 时区缺省回退环境默认（真实文案通常带 (Asia/Shanghai)，但不应强制）
