...
/** watchdog 心跳文件候选名（优先 watchdog.log，其次 cc-stats.json）。 */
const WATCHDOG_HEARTBEAT_FILES = ['watchdog.log', 'cc-stats.json'];

/**
 * 派发前 watchdog liveness 探针：判定 cc 派发链（queue → watchdog → runner）是否活着，
 * 不新鲜/结构缺失时直接返回不可用，避免上层白等整个轮询超时才发现通道早已失效。
 * @param {object} params
 * @param {object} params.fsImpl - 注入的文件系统实现，需实现 access/stat/readdir。
 * @param {string} params.root - cc-tasks 根目录。
 * @param {object} [params.env] - process.env 风格对象，用于解析 DSH_CC_WATCHDOG_STALE_MS 覆盖。
 * @param {number} [params.now] - 当前时间戳（毫秒，可注入便于测试），缺省 Date.now()。
 * @returns {Promise<{ok: boolean, reason: string, details: {logAgeMs: (number|null), queueDepth: (number|null), tasksCount: (number|null)}}>}
 */
export async function ccWatchdogAlive({ fsImpl, root, env, now }) {
  ...
  try {
    ...
    return { ok: false, reason: 'root-unavailable', details: { logAgeMs: null, queueDepth: null, tasksCount: null } };
  }

  let queueDepth = null;
  try {
    const entries = await fsImpl.readdir(path.join(root, 'queue'));
    queueDepth = Array.isArray(entries) ? entries.length : null;
  } catch (err) {
    return { ok: false, reason: 'queue-dir-missing', details: { logAgeMs: null, queueDepth: null, tasksCount: null } };
  }
  ...
  try {
    ...
    return { ok: false, reason: 'tasks-dir-missing', details: { logAgeMs: null, queueDepth, tasksCount: null } };
  }
  ...
  if (logAgeMs === null) {
    return { ok: false, reason: 'heartbeat-missing', details: { logAgeMs: null, queueDepth, tasksCount } };
  }

  if (logAgeMs > thresholdMs) {
    return {
      ok: false,
      reason: `cc-watchdog-stale:${logAgeMs}`,
      details: { logAgeMs, queueDepth, tasksCount },
    };
  }

  return { ok: true, reason: 'alive', details: { logAgeMs, queueDepth, tasksCount } };
}
