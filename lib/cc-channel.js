...
import { parseResetsAt, isQuotaFailureText, shortcutCapMs } from './quota-parser.mjs';

// 兼容既有调用方（含 test/cc-channel.test.js、lib/index.js）：parseResetsAt 的判断已收敛到
// 唯一模块 lib/quota-parser.mjs（见 ccfix-20260919-quotaclass），本文件不再自带实现，只再导出
// 同一个函数（签名不变，仍支持 { now } 注入），避免调用方改 import 路径。
export { parseResetsAt };
...
export function classifyCcFailure({ result, claudeLogText, now } = {}) {
  ...
  const isQuotaExhausted = errorCode === 'cc-quota-exhausted'
    // 措辞判据已收敛至 lib/quota-parser.mjs 的 LIMIT_WORDING（唯一模块，不再自带正则字面量），
    // 覆盖 session limit / rate limit / weekly limit / usage limit 与 hit your ... limit / quota。
    || isQuotaFailureText(combined);
  if (isQuotaExhausted) {
...
/**
 * 派发前配额状态判定：若已持久化的最近一次失败分类已知为配额耗尽、resetsAt 可解析、且
 * 剩余等待时间落在 {@link shortcutCapMs}（按 quotaKind 取 weekly 24h / 其余 6h）之内，
 * 才跳过本次派发（沿用既有降级链，不再盲等整个轮询超时）。
 *
 * cap 的存在是对 “resetsAt 被误解析 / 状态文件陈旧或写坏” 的保护：parseResetsAt 在目标
 * 挂钟时刻已过时会把它滚到明天，异常输入或陈旧文件都可能让 resetsAt 落在很远的未来。
 * 没有 cap 时，一个错误的时间戳会让插件静默停摆整条 cc 派发通道，且没有自愈手段。
 * 超出 cap 一律 fail-open 到“照常派发”——多派一次至多被 runner 重新归类为
 * cc-quota-exhausted（浪费一次尝试），代价远小于长时间静默停摆。
 *
 * cap 必须复用 lib/quota-parser.mjs 的 shortcutCapMs（与 parseResetsAt / isQuotaFailureText
 * 同一处 import），不得在本文件另写 6h/24h 常量——那正是本策略要消除的“策略重复实现”。
 * quotaState.quotaKind 缺失（旧状态文件写于本次改动之前）时按 'session' 处理（6h，保守）。
 *
 * 纯函数，可注入 now 便于测试；quotaState 缺失/字段不全一律判定为“不跳过”（fail-open，
 * 交由正常派发链自行发现问题），避免因状态文件损坏而误伤派发。
 * @param {object} params
 * @param {{kind: string, resetsAt: (string|null), quotaKind: (string|undefined)}|null|undefined} params.quotaState - 上次持久化的配额分类状态。
 * @param {number} [params.now] - 当前时间戳（毫秒，可注入），缺省 Date.now()。
 * @returns {boolean} true 表示应跳过本次派发（配额已知未恢复，且在保护性 cap 之内）。
 */
export function shouldSkipForKnownQuotaExhaustion({ quotaState, now } = {}) {
  ...
  const nowMs = typeof now === 'number' && Number.isFinite(now) ? now : Date.now();
  const remainingMs = resetsMs - nowMs;
  const capMs = shortcutCapMs(quotaState.quotaKind ?? 'session');
  return remainingMs > 0 && remainingMs <= capMs;
}
