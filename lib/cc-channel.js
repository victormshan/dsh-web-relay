 ...
 * 使“配额耗尽”“权限被拒”“真实执行超时”“代码本身失败”在审计上互相可区分（此前一律记成
 * `claude exit=N; done.flag=missing`，配额耗尽与真实代码失败在审计上不可区分）。
 * 优先消费 runner.sh 已产出的 result.json.errorCode（cc-quota-exhausted / cc-permission-denied /
 * cc-timeout / cc-failed / v2-validate-failed），errorCode 缺失时退化为对 claude.log 原文做
 * 关键词兜底识别。纯函数，不做任何 IO。
 * @param {object} params
 ...
 * @returns {{kind: ('quota-exhausted'|'permission-denied'|'timeout'|'code-failed'|'unknown'), errorCode: (string|null), resetsAt: (string|null), raw: string}}
 */
