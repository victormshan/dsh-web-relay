 ...
 *   由调用方注入 sha256）。
 * @returns {Promise<{checked: number, differing: Array<{path: string, kind: 'content-diff'|'missing-source'|'missing-runtime'}>}>}
 */
export async function computeDrift({ manifest, listDir, readFile, hash } = {}) {
  const hashFn = typeof hash === 'function' ? hash : defaultHash
  const relFiles = await expandManifest(manifest, listDir)
  const differing = []
  let checked = 0
...
export const SELFCHECK_REQUIRED_HEALTH_FIELDS = [
  ...
  'restartStats', 'channelStats', 'dialogFallbackRate', 'ccStats',
