...
// ---------------------------------------------------------------------------
// v4.11.0 步9: /ask body 显式声明与文本声明冲突时显式拒绝
// 背景：POST /dsh-web-relay/ask 此前只从 prompt+answer 文本提取 iterations/finalAcceptance/
// autoDecision，body 里的同名字段被静默忽略（实测：body 传 iterations:2/autoDecision:true，
// 落盘仍是文本解析出的 1/false）。调用方以为 body 生效了，实际被无声覆盖——这是一个"两个声明来源，
// 却只有一个生效且不告知"的缺陷。
// 修复原则：只做"检测 + 显式拒绝"，不做"合并"——合并会把 body 与文本变成两个都部分生效的来源，
// 语义比现状更难推理。body 未带这三个字段中任何一个时不做任何判断，行为与此前完全一致。
// ---------------------------------------------------------------------------
export function detectAskBodyDeclConflict(payload, textDecl) {
  const p = (payload && typeof payload === 'object' && !Array.isArray(payload)) ? payload : {}
  const t = (textDecl && typeof textDecl === 'object') ? textDecl : {}
  const fields = ['iterations', 'finalAcceptance', 'autoDecision']
  const conflicts = []
  for (const f of fields) {
    if (!Object.prototype.hasOwnProperty.call(p, f)) continue
    if (p[f] !== t[f]) conflicts.push({ field: f, body: p[f], text: t[f] })
  }
  if (conflicts.length === 0) return { conflict: false }
  const detail = conflicts
    .map((c) => `${c.field}: body 声明=${JSON.stringify(c.body)}，文本解析=${JSON.stringify(c.text)}`)
    .join('；')
  return {
    conflict: true,
    error: `body 与文本声明不一致（含"文本未声明→落为默认值"的情形）：${detail}。请只用其中一处声明——要么只在 body 显式传字段，要么只在 prompt/answer 文本写严格 JSON 声明块，不要同时使用两处。`
  }
}
