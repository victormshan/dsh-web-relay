...
// V2-2: 版间门 × 突破度联动——纯函数，决定"上一版全部 approved 后，是否放行进入下一版"。
// 入参（均可注入，函数内不做 IO/不读 env/不调用 Date）：
//   steps — 刚完成版本的 step 数组（同 versionTypeOf 输入契约），仅用于复用 versionTypeOf 识别
//           "本版含 structural/paradigm" 这一事实——不用于重新累加连增数（累加已由 auditBreakthrough/
//           evaluateBreakthroughPlan 在 restructure/创建阶段完成并落盘为 state.incrementalStreak，
//           此处若再次相加会对同一版本重复计数）。
//   state — 落盘的 stepState（读取 iterations/currentIteration/incrementalStreak，只读不改）。
//   max   — 连续 Incremental 上限；调用方负责解析 DSH_RELAY_BREAKTHROUGH_MAX_INCREMENTAL（默认 3）
//           后传入，本函数不读环境变量，便于单测注入不同阈值。
// 出参：{ advance, verdict, consecutiveIncremental, requiredType, reasons, handoffNote }
//   verdict: 'advance' | 'blocked-needs-breakthrough' | 'final-iteration' | 'single-iteration'
export function evaluateVersionAdvance({ steps, state, max } = {}) {
  const st = state || {}
  const seq = Array.isArray(steps) ? steps : []
  const iterations = Number(st.iterations) || 1
  const currentIteration = Number(st.currentIteration) || 1
  const MAX = Number.isFinite(Number(max)) ? Number(max) : 3

  if (iterations <= 1) {
    return {
      advance: true, verdict: 'single-iteration',
      consecutiveIncremental: Number(st.incrementalStreak) || 0,
      requiredType: null, reasons: [], handoffNote: null
    }
  }
  if (currentIteration >= iterations) {
    return {
      advance: true, verdict: 'final-iteration',
      consecutiveIncremental: Number(st.incrementalStreak) || 0,
      requiredType: null, reasons: [], handoffNote: null
    }
  }

  // 复用 versionTypeOf 的既有类型识别（不重复实现）：本版若含 structural/paradigm，
  // 即为"突破"，streak 归零；否则沿用落盘的 incrementalStreak（已由上游 audit 计入本版，不再相加）。
  const vt = versionTypeOf(seq)
  const consecutiveIncremental = vt === 'breakthrough' ? 0 : (Number(st.incrementalStreak) || 0)

  if (consecutiveIncremental >= MAX) {
    const reasons = [
      `本版所在的连续 Incremental 版本数已达 ${consecutiveIncremental}`,
      `已达阈值 DSH_RELAY_BREAKTHROUGH_MAX_INCREMENTAL=${MAX}，下一版须含 Structural/Paradigm 突破项方可继续推进`
    ]
    const handoffNote = `本版为第 ${consecutiveIncremental} 个连续 Incremental 版本，下一版必须包含 Structural 或 Paradigm 突破项才能继续推进；依据：${reasons.join('；')}`
    return { advance: false, verdict: 'blocked-needs-breakthrough', consecutiveIncremental, requiredType: 'structural', reasons, handoffNote }
  }
  return { advance: true, verdict: 'advance', consecutiveIncremental, requiredType: null, reasons: [], handoffNote: null }
}
