...
import { checkL1Gate, runL2ShadowGate, shouldUseShadow, resolveRepoPath, runShadowGC, executeRollbackBaseline, getGitHead, gcScheduleMs } from './shadow-gate.js'
...
export function apply(ctx) {
   ...
   */
  async function checkPendingHuman() {
    let raw = null
    try {
  ...
  const replayHandler = async (req, res) => {
    ...
    const stepUpdateHandler = async (req, res) => {
      try {
        ...
        if (action === 'start') {
          ...
          const repoRoot = resolveRepoPath(base) || (typeof payload.repoPath === 'string' ? payload.repoPath : null) || process.env.DSH_RELAY_REPO_PATH || null
        ...
        if (!poll.ok) {
          ...
          if (poll.status === 'failed') {
            ...
            if (classification.kind === 'quota-exhausted') {
              ...
              await writeCcQuotaState({ kind: 'quota-exhausted', resetsAt: classification.resetsAt, at: new Date().toISOString() })
    ...
    const finalizeHandler = async (req, res) => {
      try {
        ...
        {
          const repoBase = resolveRepoPath(base) || (typeof payload.repoPath === 'string' ? payload.repoPath : null) || process.env.DSH_RELAY_REPO_PATH || null
        ...
        {
          const gcRepo = resolveRepoPath(base) || (typeof payload.repoPath === 'string' ? payload.repoPath : null) || process.env.DSH_RELAY_REPO_PATH || null
          if (gcRepo) {
  ...
  const rollbackHandler = async (req, res) => {
    try {
      ...
      const safePolicy = safePolicyFor(base)
      const state = await readStepState(base, exprId)
      const repo = resolveRepoPath(base) || (typeof payload.repoPath === 'string' ? payload.repoPath : null) || process.env.DSH_RELAY_REPO_PATH || null
  ...
  checkPendingHuman().catch((e) => console.warn('[dsh-web-relay] [pendingHuman] boot 检查异常（fail-open）:', String((e && e.message) || e)))
  ...
  // 环境开关：DSH_RELAY_REPO_PATH 指定目标仓库（git）；DSH_RELAY_GC_MS 周期毫秒（默认 600000=10min；0/非法=关闭定时，仅保留 finalize 触发）。
  // 幂等：仅第一个 apply() 安装（进程级单例），unref 防挂住宿主退出。
  if (!scheduledGcTimer) {
    const gcMs = gcScheduleMs(process.env.DSH_RELAY_GC_MS || 600000)
    const gcRepo = (typeof process.env.DSH_RELAY_REPO_PATH === 'string' && process.env.DSH_RELAY_REPO_PATH) ? process.env.DSH_RELAY_REPO_PATH : null
    if (gcMs > 0 && gcRepo) {
      ...
      // 无 env repoPath 或周期=0：定时 GC 停用（finalize 内联 GC 不受影响）
      console.warn('[dsh-web-relay] 定时 Shadow GC 未启用（需 DSH_RELAY_REPO_PATH 指定 git 仓库，且 DSH_RELAY_GC_MS>0）；finalize 内联 GC 仍生效')
    }
  }
