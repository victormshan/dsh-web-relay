...
import { parsePendingHuman, decidePendingHuman, pendingHandoffText } from './pending-human.mjs'
...
export function apply(ctx) {
  ...
  async function checkPendingHuman() {
    ...
    if (dec.decision === 'wake') {
      // sessionId 取 DSH_SESSION_ID；无则不唤醒，只记录（fail-open，不阻断 boot/心跳）。
      const sessionId = process.env.DSH_SESSION_ID || null
      if (!sessionId) {
        error = 'sessionId 缺失（无 DSH_SESSION_ID），仅记录未唤醒'
      } else {
        try {
          const w = await wakeMainAgent({ sessionId, handoffText: pendingHandoffText(entry) })
    ...
    } else if (dec.decision === 'would-wake') {
      // dryRun：判定为需要唤醒但不得调用 wakeMainAgent（供验收探针空跑）。
      notifiedPendingHumanIds.add(entry.id)
    }
    lastPendingHuman = {
      id: entry.id,
