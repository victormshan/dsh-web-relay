  ...
  async function webGeminiAsk(prompt, timeoutMs = 300000) {
    try {
      ...
      while (Date.now() < deadline) {
        ...
        const verdict = classifyBridgeTask(task, { processingSince, stallMs: BRIDGE_STALL_MS })
        if (verdict.state === 'done') return { ok: true, text: verdict.answer }
        if (verdict.state === 'failed' || verdict.state === 'stalled') return { ok: false, error: verdict.error }
      }
