...
export function apply(ctx) {
  ...
  const replayHandler = async (req, res) => {
    ...
    async function obtainReviewVerdict(state, step, base, safePolicy, enableSwarm, reviewChannel, enableCrossCheck) {
      ...
      if (enableSwarm) {
        ...
        const runRole = async (roleId) => {
          ...
          if (!rr.ok) return { roleId, review: { verdict: 'rejected', findings: ['审核通道不可用'], suggestion: '' } }
        ...
        const sec = swarmResults.find((x) => x.roleId === 'security') || { review: { verdict: 'rejected', findings: [] } }
        const ref = swarmResults.find((x) => x.roleId === 'refactor') || { review: { verdict: 'rejected', findings: [] } }
        const consensus = swarmConsensus(sec.review.verdict, ref.review.verdict)
        ...
        return {
          ...
          fallbackReason: 'V3.2 Swarm 双角色盲审',
          manual: false,
    ...
    async function applyReviewOutcome(state, step, v, base, safePolicy) {
      const exprId = state.exprId
      const { result, reason, reviewer, reviewerLabel, fallbackReason } = v
      ...
      return { state, step, updated, result, reason, reviewer: result === 'approved' ? reviewerBy : null, reviewerLabel, fallbackReason, traceText: v.traceText }
    }
