...
export function apply(ctx) {
  ...
  const replayHandler = async (req, res) => {
    ...
    async function obtainReviewVerdict(state, step, base, safePolicy, enableSwarm, reviewChannel) {
    ...
    async function reviewOneStep(state, step, base, safePolicy, enableSwarm, reviewChannel) {
      const v = await obtainReviewVerdict(state, step, base, safePolicy, enableSwarm, reviewChannel)
      if (v.manual) {
        return {
          state, step,
          manual: true,
    ...
    const autoReviewHandler = async (req, res) => {
      try {
        ...
        const out = await reviewOneStep(state, step, base, safePolicy, swarmEnablePolicy({ importance: step.importance, enableSwarm: payload.enableSwarm }).enabled, payload.reviewChannel)
        if (out.manual) {
          // 降级到手动：前端展开审核框
          return json(res, 200, {
            ok: true,
