  ...
  const replayHandler = async (req, res) => {
    ...
    async function runCcReviewTask({ exprId, step, recordText, traceText, artifactsSummary, caseBlock }) {
      ...
      try {
        ...
        // 缺陷 3 修复：派发前 watchdog liveness 探针——queue/tasks 结构缺失或心跳陈旧时
        // 直接降级，不再白等整个轮询超时（可能长达 960000ms）才发现通道早已失效。
        ...
        if (!poll.ok) {
          ...
          const pollStatReason = poll.reason === 'timeout-still-running' ? 'timeout-still-running' : `poll ${poll.status}: ${poll.error || ''}`
          await recordCcStat({ taskId: task.taskId, kind: 'review', ok: false, elapsedMs: Date.now() - ccT0, reason: pollStatReason })
          return { ok: false, error: `claude-code 审核未完成（${poll.reason}${poll.error ? ': ' + poll.error : ''}）` }
        }
        ...
        return { ok: true, verdict: v.verdict, reason: v.reason, taskId: task.taskId }
      } catch (e) {
