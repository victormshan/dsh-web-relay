        ...
        if (!(payload.auxiliary === true) && (phase || parsedSteps.length > 0)) {
          try {
            ...
            await writeStepState(base, id, {
              ...
              autoIterDeclAudit: askAutoIterDecl,
              steps: parsedSteps
            }, safePolicy)
  ...
  const replayHandler = async (req, res) => {
    ...
    const stepsDeclareHandler = async (req, res) => {
      try {
        ...
        const updated = await writeStepState(base, exprId, { ...state, ...plan.after }, safePolicy)
        await appendTrace({ base, safePolicy, exprId, entries: [traceEntry(role, plan.traceText)] }).catch(() => {})

        const autoIterDecl = assessAutoIterDecl(plan.after)
