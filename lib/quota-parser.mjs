...
/**
 * 配额种类判定：从文案里判断这是哪一类限额（用于决定短路窗口：weekly 24h / 其余 6h）。
 * 与 {@link isQuotaFailureText} / {@link LIMIT_WORDING} 是两件事：后者只回答“是不是配额失败”，
 * 本函数回答“如果是，是哪一种”——两者不能互相替代，也不该合并成一个函数。
 * 优先级 weekly > rate > usage > session（LIMIT_WORDING 命中但不含前三个关键词）> unknown。
 * **完全不含限额措辞时必须返回 'unknown'，不得默认成 'session'**——那会把“文本里根本没有配额信息”
 * 谎报成“会话限额”，误导下游按 session 的短路窗口（6h）处理一段其实无法判定种类的文本。
 * @param {string} text - 待判定文本。
 * @returns {'weekly'|'rate'|'usage'|'session'|'unknown'}
 */
export function quotaKindOf(text) {
  const s = typeof text === 'string' ? text : '';
  if (/weekly/i.test(s)) return 'weekly';
  if (/rate/i.test(s)) return 'rate';
  if (/usage/i.test(s)) return 'usage';
  if (LIMIT_WORDING.test(s)) return 'session';
  return 'unknown';
}

/**
 * **探针语义**的细节版：在 {@link classifyQuotaProbe} 的可用性判定之外，一并给出配额种类与
 * 解析出的恢复时刻，避免消费方各自拼 kind/resetsAt（那样又是“同一判断多处实现”）。
 * @param {string} text - 探针原始输出。
 * @returns {{available: boolean, kind: 'weekly'|'rate'|'usage'|'session'|'unknown', resetsAt: string|null}}
 */
export function classifyQuotaProbeDetail(text) {
  return {
    available: classifyQuotaProbe(text) === 'available',
    kind: quotaKindOf(text),
