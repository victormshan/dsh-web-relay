window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- ccfeat-20260922-protoversion: 协议版本——数据驱动，不再手写版本列表 ----
    // 唯一权威来源是 /dsh-web-relay/context（及 /protocol）返回的 protocolVersions[]
    // （lib/index.js PROTOCOL_VERSIONS_META）。下面这份 FALLBACK 只在网络不可达时用于
    // 渲染只读兜底副本，绝不作为运行期唯一来源；version/contentKey 由数字动态拼出，
    // 源码里不出现整串连续版本号字面量列表（那是旧世界缺陷的机器签名）。
    const PROTOCOL_VERSION_FALLBACK_DEFS = [
      [15, false, '线性模式'],
      [16, true, 'DAG 并发调度'],
      [17, true, '智能编排'],
      [18, true, '混合模式'],
    ...
    const formatProtocolVersion = (n) => 'v' + (n / 10).toFixed(1)
    const PROTOCOL_VERSIONS_FALLBACK = PROTOCOL_VERSION_FALLBACK_DEFS.map(([n, concurrent, label]) => ({
      version: formatProtocolVersion(n),
      concurrent,
      contentKey: 'protocolV' + n,
      label: formatProtocolVersion(n) + ' ' + label
    }))
    // 5 段式打包模板自 v1.7 起引入、随后各版本沿用（按 contentKey 判定，不比较 protocolVersion 字面量）。
    const TEMPLATE_CONTENT_KEYS = new Set(['protocolV17', 'protocolV18', 'protocolV19', 'protocolV20'])
    // resolveProtocolVersion(stored, meta): stored 命中元数据 → 该 version；未命中/缺失 → 元数据首项；meta 为空 → null。
    // 刻意不写死 'v1.5' 兜底——回落目标由数据决定，元数据换了首项这里也跟着变。
    const resolveProtocolVersion = (stored, meta) => {
      const list = Array.isArray(meta) ? meta : []
      if (list.length === 0) return null
      const hit = list.find((m) => m && m.version === stored)
      return hit ? hit.version : list[0].version
    }
    ...
    const DOCK_CSS = '' +
      ...
      '@keyframes dwr-blink{0%,100%{opacity:1}50%{opacity:.25}}'
    ...
    const localeDict = {
      zh: {
        ...
        // v3.9 S4: watchdog/优雅停机准备状态提示（/health-check.preparing）
        preparingRestart: '优雅停机准备中（重启通道已就绪）',
        // v4.1.0 (实验 V1): 宿主托管/重启续跑状态文案
        bootIdLabel: 'boot:',
        resumeCountLabel: '重启续接',
        resumeActiveNote: '宿主已重启·自动续跑',
        resumePausedNote: '重启续跑熔断·已跨重启',
        ...
        providerClaude: 'Claude API（Anthropic）',
        claudeHint: '任务 prompt 将经 harness llm 服务调用 Claude（需配置 ANTHROPIC_API_KEY + anthropic 路由）：',
        providerWebGemini: 'Gemini 网页版（web-gemini）',
        webGeminiHint: '任务将发到本地中转（Chrome 扩展 dsh-web-gemini-ext 自动在 gemini.google.com 回复，免配额）：',
        architectOn: '🧭 架构探讨开',
        architectOff: '🧭 架构探讨',
        protocolExpand: '▶ 三方协议',
        protocolCollapse: '▼ 三方协议',
        planningCardTitle: '当前处于 Planning 阶段',
        planningCardHint: '将外部 AI 的 executing 回复粘贴到下方输入框并点击「解析并执行」；或先点击「进入执行阶段」再继续。',
        planningPastePlaceholder: '粘贴外部 AI 的 executing 回复（含 json:agent-action 或 steps）…',
        ...
        // v2.0-1: 熔断状态卡片与 Version Gate
        circuitBreakerTitle: '⛔ 熔断已触发（连续打回 ≥3 次，自动暂停）',
        versionGateLabel: '版本',
        badgePaused: '已熔断（paused）',
        dagPreviewTitle: 'DAG 拓扑（点击节点定位步骤；low 步骤展开明细）',
        webhookPlaceholder: 'Webhook URL（如 Lark/钉钉/飞书，可选）',
        webhookSaved: 'Webhook 已保存（后端需 DSH_RELAY_WEBHOOK_URL 或重启生效）',
        webhookCleared: 'Webhook 已清空',
        webhookNeedUrl: '请先填写 Webhook URL',
        webhookTestOk: 'Webhook 测试发送成功',
        webhookTestFail: 'Webhook 测试发送失败:',
        notifyGranted: '桌面通知已开启',
        notifyDenied: '桌面通知被拒绝',
        notifyUnsupported: '浏览器不支持桌面通知',
        notificationBreakerTitle: '⛔ dsh-web-relay 熔断已触发',
        replayTitle: '🔁 Trace Replay',
        replayTimelineLabel: '状态流转（timeline）',
        replayTraceLabel: '相关三方发言（trace）',
        badgeRejected: '已打回',
        replayPickStep: '选择步骤…',
        reviewChannelTitle: '审核通道：auto=API 优先（失败依次切网页/本地 Claude Code）；web-gemini=强制网页；claude-code=强制本地 Claude Code（混合架构）',
        reviewChannelAuto: 'auto（API 优先）',
        reviewChannelWebGemini: 'web-gemini（网页）',
        reviewChannelClaudeCode: 'claude-code（本地 Claude Code）',
        swarmLabel: '双角色盲审',
        swarmHint: 'Swarm：Security-Auditor + Refactoring-Architect 双角色盲审（约 2× 审核成本）；importance:high 默认开启，可关闭',
        shadowOffLabel: '关闭影子门禁',
        shadowOffHint: 'v3.6.0 影子沙盒支撑：complete 时对 high/源码/重构步做 L1 语法预检（必要时 L2 worktree 校验），失败拒绝 complete；勾选可全局关闭',
        ...
        rollback: '回滚到上版基线', rollbacking: '回滚中…',
        rollbackConfirm: '确认回滚？将 git reset 到迭代基线（非 git 工作区将给出降级提示）',
        rollbackDone: '已回滚到基线', rollbackDegraded: '非 git 降级提示',
        // v3.8 Step4: 回滚状态展示增强文案
        rollbackBaseLabel: '回滚基线', rollbackHistory: '已回滚到',
        rollbackStepsReset: '步已复位（需重跑重审）', noRollbackBase: '无回滚基线（尚未收口记录基线，无法物理回滚）',
          enterExec: '进入执行阶段',
          parsePlanning: '解析并执行',
          webhookSave: '保存', webhookTest: '测试', notifyOn: '🔔 桌面通知',
          replayRun: '🔁 Replay',
        ...
        reviewerDialog: '对话模型',
        reviewerManual: '手动',
        reviewerClaudeCode: 'Claude Code',
        ...
        // ccfeat-20260922-protoversion: 协议版本下拉选项/标签改由 /context.protocolVersions[].label
        // 数据驱动渲染（见 PROTOCOL_VERSIONS_FALLBACK / resolveProtocolVersion）；此处只保留与版本
        // 无关的标题文案 + 按 contentKey 索引的 directive 全集（供打包正文选注入哪句版本说明）。
        protoTitle: '协议版本选择（按所选版本生成对应 Step List / 打包正文）',
        protoDirective: {
          protocolV15: '【协议 v1.5 线性执行】请按线性步骤生成 steps 数组，主 agent 将按顺序逐一执行。',
          protocolV16: '【协议 v1.6 并发调度】请分析模块独立性，对无依赖冲突的步骤显式声明 parallel_group 与 depends_on，指导主 agent 并发调度。',
          protocolV17: '【协议 v1.7 智能编排】在 v1.6 并发调度基础上，每个步骤可携带 importance（高/中/低）与 alternatives（候选方案），主 agent 按优先级与风险自主编排执行顺序。',
          protocolV18: '【协议 v1.8 混合模式】importance 升级为执行/审核分工契约：low=主 agent 直做免审（complete 自动 approved，reviewedBy=mainagent 留审计）、medium=批量轻审、high=三方严格审；review:false 为硬开关，显式指定则无条件绕过审核。',
          protocolV19: '【协议 v1.9 自动迭代】用户可声明 {"iterations":N, "finalAcceptance":"...", "autoDecision":true} 自动演进 N 版（版间门 Vn+1、连续打回≥3 熔断）；Gemini 限流时全角色自动降级内部对话模型（external→dialog→pause）。',
          protocolV20: '【协议 v2.0 本地降级链】全角色降级链新增本地 claude-code 通道（external→web-gemini→claude-code→dialog→manual）；支持 alternatives 多方案裁决与 batchStepIds 受控并发审核，遇 429 自动退避。'
        },
        depGroup: '组',
        depOn: '依赖',
        progressReady: '就绪',
        ...
        // v1.9: AutoIteration 自动迭代 + 全角色降级链（directive 文案见上方 protoDirective）
      },
      en: {
        ...
        // v3.9 S4: watchdog/graceful-restart readiness hint (/health-check.preparing)
        preparingRestart: 'Graceful restart preparing (watchdog channel ready)',
        // v4.1.0 (exp V1): host-managed / cross-restart resume labels
        bootIdLabel: 'boot:',
        resumeCountLabel: 'resume',
        resumeActiveNote: 'host restarted · auto-resumed',
        resumePausedNote: 'resume circuit-broken ×',
        ...
        providerClaude: 'Claude API (Anthropic)',
        claudeHint: 'The task prompt will be sent to Claude via the harness llm service (requires ANTHROPIC_API_KEY + anthropic route):',
        providerWebGemini: 'Gemini Web (web-gemini)',
        webGeminiHint: 'The task will be sent to the local bridge (Chrome ext dsh-web-gemini-ext replies in gemini.google.com, quota-free):',
        ...
        planningCardHint: 'Paste the external AI executing reply below and click "Parse & execute"; or click "Enter execution phase" to continue.',
        planningPastePlaceholder: 'Paste the external AI executing reply (with json:agent-action or steps)…',
        ...
        // v2.0-1: 熔断状态卡片与 Version Gate
        circuitBreakerTitle: '⛔ Circuit breaker tripped (≥3 consecutive rejects, auto-paused)',
        versionGateLabel: 'Version',
        badgePaused: 'Paused (circuit open)',
        dagPreviewTitle: 'DAG topology (click to locate; low steps expand)',
        webhookPlaceholder: 'Webhook URL (Lark/DingTalk/Feishu, optional)',
        webhookSaved: 'Webhook saved (backend needs DSH_RELAY_WEBHOOK_URL or restart)',
        webhookCleared: 'Webhook cleared',
        webhookNeedUrl: 'Enter a Webhook URL first',
        webhookTestOk: 'Webhook test sent',
        webhookTestFail: 'Webhook test failed:',
        notifyGranted: 'Desktop notifications enabled',
        notifyDenied: 'Desktop notifications denied',
        notifyUnsupported: 'Desktop notifications unsupported',
        notificationBreakerTitle: '⛔ dsh-web-relay circuit breaker tripped',
        replayTitle: '🔁 Trace Replay',
        replayTimelineLabel: 'State flow (timeline)',
        replayTraceLabel: 'Related trace entries',
        replayPickStep: 'Pick a step…',
        reviewChannelTitle: 'Review channel: auto=API first (falls back to web then local Claude Code); web-gemini=force web; claude-code=force local Claude Code (hybrid)',
        reviewChannelAuto: 'auto (API first)',
        reviewChannelWebGemini: 'web-gemini (web)',
        reviewChannelClaudeCode: 'claude-code (local Claude Code)',
        swarmLabel: 'Swarm dual review',
        swarmHint: 'Swarm: Security-Auditor + Refactoring-Architect dual blind review (~2× review cost); default ON for importance:high',
        shadowOffLabel: 'Disable shadow gate',
        shadowOffHint: 'v3.6.0 shadow support: L1 syntax precheck (L2 worktree when needed) on complete for high/source/refactor steps; failure blocks complete; check to disable globally',
        ...
        rollback: 'Rollback to base', rollbacking: 'Rolling back…',
        rollbackConfirm: 'Confirm rollback? git reset to iteration base (non-git workspace will show degrade hint)',
        rollbackDone: 'Rolled back to', rollbackDegraded: 'Non-git degrade',
        // v3.8 Step4: rollback status display strings
        rollbackBaseLabel: 'Base', rollbackHistory: 'Rolled back to',
        rollbackStepsReset: 'step(s) reset (re-run & re-review required)', noRollbackBase: 'No base recorded (not finalized yet) — physical rollback unavailable',
          enterExec: 'Enter execution phase',
          parsePlanning: 'Parse & execute',
          webhookSave: 'Save', webhookTest: 'Test', notifyOn: '🔔 Notify',
          replayRun: '🔁 Replay',
        ...
        // ccfeat-20260922-protoversion: dropdown options/labels now render from
        // /context.protocolVersions[].label (data-driven); keep only the version-agnostic
        // title text + a contentKey-indexed directive map (used when packing context).
        protoTitle: 'Protocol version (drives Step List / packed context for the selected version)',
        protoDirective: {
          protocolV15: '[Protocol v1.5 Linear] Generate a linear steps array; the main agent executes in order.',
          protocolV16: '[Protocol v1.6 DAG] Analyze module independence; mark conflict-free steps with parallel_group + depends_on to enable parallel scheduling.',
          protocolV17: '[Protocol v1.7 Smart Orchestration] On top of v1.6 DAG scheduling, each step may carry importance (high/medium/low) and alternatives; the main agent orchestrates execution order by priority and risk.',
          protocolV18: '[Protocol v1.8 Hybrid] importance becomes an execution/review duty contract: low = main agent executes directly without review (complete auto-approves, reviewedBy=mainagent kept for audit), medium = lightweight batch review, high = strict three-party review; review:false is a hard switch — explicitly setting it bypasses review unconditionally.',
          protocolV19: '[Protocol v1.9 AutoIteration] The user may declare {"iterations":N, "finalAcceptance":"...", "autoDecision":true} to auto-evolve N versions (round gates Vn+1, consecutive-rejection ≥3 circuit breaker); on Gemini rate limits every role auto-falls back to the internal dialog model (external→dialog→pause).',
          protocolV20: '[Protocol v2.0 Local Fallback Chain] The full-role fallback chain adds a local claude-code channel (external→web-gemini→claude-code→dialog→manual); supports alternatives multi-plan arbitration and batchStepIds controlled-concurrency review with 429 backoff.'
        },
        depGroup: 'Group',
        depOn: 'depends',
        progressReady: 'ready',
        ...
        // v1.9: AutoIteration + full-role fallback chain (directive text lives in protoDirective above)
      }
    }
    ...
    const RelayPanel = (props) => {
      ...
      const [health, setHealth] = useState(null)   // v2.4-2: 三端 Health Checker 状态
      const [planningPaste, setPlanningPaste] = useState('')   // v2.1.0: planning 阶段粘贴 executing 回复
      // v2.6-2: Webhook 通知配置（localStorage 持久化）
      const [webhookUrl, setWebhookUrl] = useState(() => {
        try { return localStorage.getItem('dsh-web-relay:webhook-url') || '' } catch (e) { return '' }
      })
      // v3.2.4: 审核通道选择（auto=API 优先+网页兜底；web-gemini=强制网页审核，过程可见）
      const [reviewChannel, setReviewChannel] = useState(() => {
        try { return localStorage.getItem('dsh-web-relay:review-channel') === 'web-gemini' ? 'web-gemini' : 'auto' } catch (e) { return 'auto' }
      })
      useEffect(() => {
        try { localStorage.setItem('dsh-web-relay:review-channel', reviewChannel) } catch (e) { /* ignore */ }
      }, [reviewChannel])
      // P2(v3.5.0): Swarm 双角色盲审开关（默认开；importance:high 生效；成本提示见 swarmHint）
      const [swarmOn, setSwarmOn] = useState(() => {
        try { return localStorage.getItem('dsh-web-relay:swarm-on') !== '0' } catch (e) { return true }
      })
      useEffect(() => {
        try { localStorage.setItem('dsh-web-relay:swarm-on', swarmOn ? '1' : '0') } catch (e) { /* ignore */ }
      }, [swarmOn])
      // v3.6.0 Step3: 影子沙盒门禁全局开关（默认启用门禁；勾选=关闭，供极端卡流程时）
      const [shadowOff, setShadowOff] = useState(() => {
        try { return localStorage.getItem('dsh-web-relay:shadow-off') === '1' } catch (e) { return false }
      })
      useEffect(() => {
        try { localStorage.setItem('dsh-web-relay:shadow-off', shadowOff ? '1' : '0') } catch (e) { /* ignore */ }
      }, [shadowOff])
      // v2.5-2 (UI): Trace Replay 还原结果
      const [replayResult, setReplayResult] = useState(null)
      const [replayStepId, setReplayStepId] = useState('')   // 独立 Replay 入口选中的步骤 id
      const runReplay = async (stepId) => {
        if (!exprId) { setError('缺少 exprId，无法 Replay'); return }
        setError(''); setStepBusy(true)
        try {
          const resp = await fetch('/dsh-web-relay/replay', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ workspacePath, exprId, stepId })
          })
          const data = await resp.json().catch(() => ({ ok: false, error: 'bad response' }))
          if (!data.ok) { setError(data.error || 'Replay 失败'); return }
          setReplayResult(data)
        } catch (e) { setError(String((e && e.message) || e)) }
        finally { setStepBusy(false) }
      }
      const saveWebhook = () => {
        try { localStorage.setItem('dsh-web-relay:webhook-url', webhookUrl.trim()) } catch (e) { /* ignore */ }
        setError(webhookUrl.trim() ? T.webhookSaved : T.webhookCleared)
      }
      const testWebhook = async () => {
        const url = webhookUrl.trim()
        if (!url) { setError(T.webhookNeedUrl); return }
        try {
          const r = await fetch(url, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ event: 'test', exprId: null, stepId: null, ts: new Date().toISOString(), payload: { note: 'dsh-web-relay webhook test' } })
          })
          setError(r.ok ? T.webhookTestOk : T.webhookTestFail + ' HTTP ' + r.status)
        } catch (e) { setError(T.webhookTestFail + ' ' + String((e && e.message) || e).slice(0, 60)) }
      }
      const requestNotify = () => {
        if (typeof Notification !== 'undefined') {
          Notification.requestPermission().then((p) => setError(p === 'granted' ? T.notifyGranted : T.notifyDenied))
        } else setError(T.notifyUnsupported)
      }
      ...
      // ccfeat-20260922-protoversion: 协议版本元数据（单一来源，随 /protocol、/context 下发）；
      // 未就绪（尚未 fetch 成功）时下拉/校验/正文取值一律读 PROTOCOL_VERSIONS_FALLBACK 只读兜底副本。
      const [protocolVersions, setProtocolVersions] = useState(null)
      ...
      const [showGuard, setShowGuard] = useState(false)
        ...
        // v2.6-2: 熔断桌面通知（status 变为 paused 时弹出一次）
        // 注意：依赖数组 [stepState] 同步求值，必须位于 stepState 声明之后（否则 TDZ ReferenceError）
        const prevPausedRef = useRef(false)
        useEffect(() => {
          if (!stepState) return
          const paused = stepState.status === 'paused'
          if (paused && !prevPausedRef.current) {
            try {
              if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
                new Notification(T.notificationBreakerTitle, {
                  body: (stepState.stopReason || '连续打回 ≥3 次，自动暂停') + (stepState.rejectStreak ? '（rejectStreak: ' + String(stepState.rejectStreak) + '）' : '')
                })
              }
            } catch (e) { /* ignore */ }
          }
      ...
      const [dagFocus, setDagFocus] = useState(null)   // v2.1.1: DAG 点击定位高亮的步骤 id
        const [altOpen, setAltOpen] = useState({})
        // ccfeat-20260922-protoversion: 协议版本选择——数据驱动（命中元数据即采用，不再是白名单），
        // localStorage 持久化不变。启动时元数据尚未从网络到达，先用只读兜底副本解析。
        const [protocolVersion, setProtocolVersion] = useState(() => {
          let stored = null
          try { stored = localStorage.getItem('dsh-web-relay:protocol-version') } catch (e) { /* ignore */ }
          return resolveProtocolVersion(stored, PROTOCOL_VERSIONS_FALLBACK)
        })
      ...
      // ccfeat-20260922-protoversion: 权威元数据到达后重新解析一次当前选择——
      // 若挂载时用的只读兜底副本与后端真实元数据有出入，这里回正，而不是让面板卡在兜底态。
      useEffect(() => {
        if (!Array.isArray(protocolVersions) || protocolVersions.length === 0) return
        setProtocolVersion((cur) => resolveProtocolVersion(cur, protocolVersions))
      }, [protocolVersions])
      ...
      useEffect(() => {
        ...
        fetch('/dsh-web-relay/protocol')
          ...
          .then((d) => {
            ...
            // ccfeat-20260922-protoversion: 存整份响应（含各 protocolV1x/V20 键），正文取值
            // 一律按 contentKey 通用查表，不再逐版本手写字段名。
            setProtocol(d)
            if (Array.isArray(d.protocolVersions) && d.protocolVersions.length > 0) setProtocolVersions(d.protocolVersions)
          })
          .catch(() => { /* fallback: 下拉/正文改用 PROTOCOL_VERSIONS_FALLBACK 只读兜底渲染，不白屏 */ })
      ...
      // v2.4-2: 三端 Health Checker——面板打开拉取 /health-check，每 30s 刷新
      useEffect(() => {
        if (!open) return
        let cancelled = false
        const fetchHealth = () =>
          fetch('/dsh-web-relay/health-check')
            .then((r) => (r.ok ? r.json() : null))
            .then((d) => { if (!cancelled && d && d.ok) setHealth(d) })
            .catch(() => { /* silent */ })
        fetchHealth()
      ...
      useEffect(() => {
        ...
        // v4.2.0 (实验 V2): 重启续跑/熔断状态变更审计日志（console，不影响渲染）
        useEffect(() => {
          if (!stepState) return
          const sc = stepState.restartCount || 0
          if (sc > 0 && stepState.status !== 'paused') {
            console.log('[dsh-web-relay] 重启续跑状态:', (stepState.exprId || 'expr'), 'restartCount=' + sc, 'status=' + stepState.status)
          }
          if (stepState.status === 'paused' && String(stepState.stopReason || '').includes('重启续跑熔断')) {
            console.warn('[dsh-web-relay] 重启续跑熔断暂停:', String(stepState.stopReason || '').slice(0, 140))
          }
        }, [stepState && stepState.updatedAt, stepState && stepState.status, stepState && stepState.restartCount])
      ...
      const packContext = async () => {
        ...
        try {
          ...
          // ccfeat-20260922-protoversion: 正文取值按 contentKey 通用查表（元数据 → /context →
          // /protocol → 硬编码兜底），不再逐版本 if/三元链；新增版本只需元数据多一项即自动生效。
          const localeCtx = (locale === 'en' && data.en) ? data.en : null
          const versionMeta = (Array.isArray(protocolVersions) && protocolVersions.length > 0) ? protocolVersions : PROTOCOL_VERSIONS_FALLBACK
          const activeMeta = versionMeta.find((m) => m.version === protocolVersion) || versionMeta[0]
          const contentKey = activeMeta ? activeMeta.contentKey : 'protocolV15'
          const ctxEntry = (localeCtx ? localeCtx[contentKey] : data[contentKey]) || null
          const protoLocale = (locale === 'en' && protocol && protocol.en) ? protocol.en : protocol
          const protoEntry = protoLocale ? protoLocale[contentKey] : null
          let protocolText = ''
          if (ctxEntry && ctxEntry.text) {
            protocolText = ctxEntry.text
          } else if (protoEntry && protoEntry.text) {
            protocolText = protoEntry.text
          ...
          const skillText = (ctxEntry && ctxEntry.skill && ctxEntry.skill.text) || (protoEntry && protoEntry.skill && protoEntry.skill.text) || (data.skill && data.skill.text) || null
          const lines = [
            '【三方协作机制（web-relay 语境，每次回答都必须遵循）】',
            T.protoDirective[contentKey] || T.protoDirective.protocolV15,
            ...protocolText.split('\n'),
            // v1.7 T5 / v1.8: 5 段式打包模板（v1.7/v1.8 注入：data_schema / pricing_map / mount_points / runtime_limits / history_trace，键名固定严禁省略）
            ...(TEMPLATE_CONTENT_KEYS.has(contentKey) ? [
              '',
              '【' + T.tmplHeader + '】',
        ...
        try {
          ...
          setSavedPath(data.savedPath || '')
            if (data.fallbackReason) {
              setError('⚠ 自动降级提示：' + data.fallbackReason + '（本次由 ' + (data.providerLabel || data.channel || '外部通道') + ' 完成回答）')
            }
            setExprId(data.id || '')
          setPhase('done')
        } catch (e) { setError(String(e?.message || e)) }
      ...
      const execute = async (indices) => {
        ...
        const autoReviewStep = async (stepId) => {
          ...
          try {
            const resp = await fetch('/dsh-web-relay/steps/auto-review', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ workspacePath, exprId, stepId, sessionId, protocolVersion, enableSwarm: swarmOn, disableShadow: shadowOff, reviewChannel }),
              signal: controller.signal
            })
            const data = await resp.json().catch(() => ({ ok: false, error: 'bad response' }))
            if (!data.ok) {
        ...
        const batchAutoReview = async () => {
          ...
          try {
            const resp = await fetch('/dsh-web-relay/steps/auto-review', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ workspacePath, exprId, sessionId, protocolVersion, batchStepIds: ids, enableSwarm: swarmOn, disableShadow: shadowOff, reviewChannel }),
              signal: controller.signal
            })
            const data = await resp.json().catch(() => ({ ok: false, error: 'bad response' }))
            if (!data.ok) {
        ...
        // v3.7.0 P2 Step3: 回滚到上版基线（POST /steps/rollback；非 git 降级提示）
        const rollbackToBase = async () => {
          if (!exprId) return
          if (!window.confirm(T.btn.rollbackConfirm)) return
          setStepBusy(true); setError(''); setStepBusyAction('rollback')
          try {
            const resp = await fetch('/dsh-web-relay/steps/rollback', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ workspacePath, exprId, protocolVersion, repoPath: 'D:\\DSH' })
            })
            const data = await resp.json().catch(() => ({ ok: false, error: 'bad response' }))
            if (!data.ok) {
              // v3.8 Step4: 缺基线/降级结果给出明确可读提示（含落盘降级标记时的确认）
              setError(((data.degraded ? T.btn.rollbackDegraded + '：' : '') + (data.reason || data.error || 'rollback failed')) + (data.marked ? '（已落盘 rollbackDegraded 标记）' : ''))
              return
            }
            // v3.8 Step4: 回滚成功显示基线 + 复位步数（resetInfo 由 v3.8 Step3 端点返回）
            const base12 = String(data.base || '').slice(0, 12)
            const resetTxt = data.resetInfo && data.resetInfo.stepsReset ? ` · ${data.resetInfo.stepsReset}${T.btn.rollbackStepsReset}` : ''
            setError(T.btn.rollbackDone + ' ' + base12 + resetTxt)
            refreshStepState()
          } catch (e) { setError(String(e?.message || e)) }
          finally { setStepBusy(false); setStepBusyAction('') }
        }
        ...
        const setExperimentPhase = async (nextPhase) => {
          if (!exprId) { setError('缺少 exprId（请先发起任务），无法切换阶段'); return }
          const stepId = (stepState && stepState.currentStep) || (steps && steps[0] && steps[0].id)
          if (!stepId) { setError('缺少 stepId（Step List 为空），无法切换阶段'); return }
          setStepBusy(true); setError('')
          try {
            const resp = await fetch('/dsh-web-relay/steps/update', {
              method: 'POST',
        ...
        // v1.5.1: 面板打开时拉取 context 记录列表（实验选择下拉数据源），
        // 确保"最新任务"被截断/误判时用户仍可精确切换到任意任务查看 Step List。
        useEffect(() => {
          if (!open) return
          let cancelled = false
          fetch('/dsh-web-relay/context?cwd=' + encodeURIComponent(workspacePath || ''))
            .then((r) => (r.ok ? r.json() : null))
            .then((d) => { if (!cancelled && d && d.ok && Array.isArray(d.records)) setContextRecords(d.records) })
            .catch(() => { /* ignore */ })
          return () => { cancelled = true }
        }, [open, workspacePath])
      ...
      // v1.9: 统一"提问"通道（gemini-free / claude / web-gemini）——走 /ask，provider 参数化
      const submitAsk = async (provider) => {
        if (!prompt.trim()) { setError('请填写提问内容'); return }
        setError(''); setSending(true); setAnswer(''); setSavedPath(''); setPhase('working')
        try {
          const finalPrompt = provider === 'gemini-free' && architectMode ? '[MODE: ARCHITECT_PLANNING_v1.4]\n' + prompt : prompt
          const resp = await fetch('/dsh-web-relay/ask', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ provider, prompt: finalPrompt, answer: '', workspacePath, sessionId, protocolVersion })
          })
          const data = await resp.json().catch(() => ({ ok: false, error: 'bad response' }))
          if (!data.ok) { setError(data.error || 'request failed'); return }
          setAnswer(data.answer || '')
      ...
      const onEnterSubmit = (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {
          e.preventDefault()
          if (provider === 'gemini-free' || provider === 'claude' || provider === 'web-gemini') submitAsk(provider)
      ...
      const checkedIndices = actions
        ...
        // v2.1-2: DAG 拓扑分层（无依赖=0 层；depends_on 取依赖层 max+1；parallel_group 强制同层）
        const dagLayers = (list) => {
        ...
        const dagPreviewLayers = steps && steps.length > 0 ? dagLayers(steps) : []
      ...
      let footer = null
      if (phase !== 'preview' && (provider === 'gemini-free' || provider === 'claude' || provider === 'web-gemini')) {
        const stopped = stepState && stepState.status === 'stopped'
        footer = h('div', { style: { display: 'flex', gap: 8, alignItems: 'center', width: '100%' } },
          h('button', { onClick: () => submitAsk(provider), disabled: sending, style: { ...btnStyle, opacity: sending ? 0.6 : 1 } },
      ...
      return h('div', {
        ...
        h('div', { style: panelBodyStyle },
          config && !config.geminiConfigured
            ...
            : (config && h('div', { style: hintStyle }, `${T.configVersion}${config.version || '?'} · ${config.geminiConfigured ? T.configReady + ' · model: ' + config.model : T.configNotReady} · shell: ${config.shellAvailable ? '✓' : '✗'} · apiProxy: ${config.apiProxyAvailable ? '✓' : '✗'} · claude API: ${config.claudeConfigured ? '✓' : '✗（缺 ANTHROPIC_API_KEY）'}`)),
          // v2.4-2: 三端 Health Checker 状态灯（bridge 红/绿；扩展与 Gemini 页面由面板客户端探测占位灰）
          health && h('div', { style: { display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', margin: '2px 0', fontSize: 11, color: '#a1a1aa' } },
            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 4 } },
              h('span', { style: { width: 8, height: 8, borderRadius: 4, background: (health.bridge && health.bridge.ok) ? '#4ade80' : '#ef4444' } }), 'bridge'),
            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 4 } },
              h('span', { style: { width: 8, height: 8, borderRadius: 4, background: '#71717a' } }), 'ext'),
            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 4 } },
              h('span', { style: { width: 8, height: 8, borderRadius: 4, background: '#71717a' } }), 'gemini'),
            h('span', {}, T.configVersion + (health.version || '?')),
            // v4.1.0 (实验 V1): bootId 短显（宿主进程标识，重启前后可对比）
            h('span', { style: { color: '#71717a', fontFamily: 'monospace' } }, T.bootIdLabel + (health.bootId ? String(health.bootId).slice(-8) : '?')),
            // v3.9 S4: 优雅停机准备中状态灯（琥珀；watchdog 探测通道就绪提示）
            health.preparing && h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 4, color: '#fbbf24', fontWeight: 600 } },
              h('span', { style: { width: 8, height: 8, borderRadius: 4, background: '#fbbf24', animation: 'dwr-blink 1s infinite' } }), T.preparingRestart)
          ),
          // ccfeat-20260922-protoversion: 协议版本选择 + 可展开全文——数据驱动（下拉/正文取值
          // 均来自 protocolVersions 元数据；/context·/protocol 尚未到达时用只读兜底副本渲染，不白屏）。
          protocol && (() => {
            const meta = (Array.isArray(protocolVersions) && protocolVersions.length > 0) ? protocolVersions : PROTOCOL_VERSIONS_FALLBACK
            const activeMeta = meta.find((m) => m.version === protocolVersion) || meta[0]
            const contentKey = activeMeta ? activeMeta.contentKey : 'protocolV15'
            const hasLoadedContent = Boolean(protocol && protocol[contentKey])
            const expandLabelVersion = hasLoadedContent ? protocolVersion : (meta[0] ? meta[0].version : protocolVersion)
            const active = (locale === 'en' && protocol.en) ? protocol.en : protocol
            const pick = active ? active[contentKey] : null
            return h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, margin: '2px 0' } },
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
                h('select', {
                  ...
                  meta.map((m) => h('option', { key: m.version, value: m.version }, m.label))
                ),
                h('button', {
                ...
                }, (showProtocol ? T.protocolCollapse : T.protocolExpand) + ' ' + expandLabelVersion)
              ),
              showProtocol && h('div', { style: { ...preStyle, marginTop: 0, maxHeight: 160, fontSize: 12, width: '100%', boxSizing: 'border-box' } },
                (pick && pick.text) ? pick.text : protocol.text)
            )
          })(),
          // v2.6-2: Webhook 通知配置（URL + 保存 + 测试）+ 桌面通知开关
          h('div', { style: { display: 'flex', gap: 6, alignItems: 'center', margin: '2px 0', flexWrap: 'wrap' } },
            h('input', {
              value: webhookUrl,
              onChange: (e) => setWebhookUrl(e.target.value),
              placeholder: T.webhookPlaceholder,
              style: { ...inputStyle, margin: 0, flex: 1, fontSize: 11, minWidth: 140 }
            }),
            h('button', { onClick: saveWebhook, className: 'dwr-ghost', style: { ...btnGhostStyle, padding: '2px 8px', fontSize: 11 } }, T.btn.webhookSave),
            h('button', { onClick: testWebhook, className: 'dwr-ghost', style: { ...btnGhostStyle, padding: '2px 8px', fontSize: 11 } }, T.btn.webhookTest),
            h('button', { onClick: requestNotify, className: 'dwr-ghost', style: { ...btnGhostStyle, padding: '2px 8px', fontSize: 11 } }, T.btn.notifyOn)
          ),
          h('select', {
            value: provider,
            ...
            h('option', { value: 'claude' }, T.providerClaude),
            h('option', { value: 'web-gemini' }, T.providerWebGemini)
          ),
          provider === 'manual'
            ? h('div', { style: hintStyle }, T.manualHint)
            : provider === 'claude'
              ? h('div', { style: hintStyle }, T.claudeHint)
              : provider === 'web-gemini'
                ? h('div', { style: hintStyle }, T.webGeminiHint)
          ...
          needsAgent && h('div', { style: { marginTop: 8, border: '1px solid #f59e0b', borderRadius: 6, padding: 8, background: 'rgba(245,158,11,.08)' } },
            ...
            // v1.3: always allow loading an existing Step List by expr id,
              ...
              // v2.1.0: 无论 provider 都显示专用粘贴框（修复原提示指向不存在的 manual 输入框）
              stepState && stepState.phase === 'planning' && h('div', { style: { marginTop: 8, border: '1px solid #f59e0b', borderRadius: 6, padding: 8, background: 'rgba(245,158,11,.06)' } },
                ...
                h('textarea', {
                  value: planningPaste,
                  onChange: (e) => setPlanningPaste(e.target.value),
                  placeholder: T.planningPastePlaceholder,
                  rows: 5, style: { ...inputStyle, marginTop: 6 }
                }),
                h('div', { style: { display: 'flex', gap: 6, marginTop: 6, alignItems: 'center' } },
                  h('button', {
                    onClick: () => {
                      if (!planningPaste.trim()) { setError(T.manualNeedPaste); return }
                      setProvider('manual'); setPasted(planningPaste)
                      submitManual()
                    },
                    disabled: stepBusy || !planningPaste.trim(),
                    style: { ...btnStyle, padding: '4px 10px', fontSize: 12, opacity: (stepBusy || !planningPaste.trim()) ? 0.6 : 1 }
                  }, T.btn.parsePlanning),
                  h('button', { onClick: () => setExperimentPhase('executing'), disabled: stepBusy, style: { ...btnStyle, padding: '4px 10px', fontSize: 12 } }, T.btn.enterExec)
                )
              ),
            ...
            stepState && steps && steps.length > 0 && h('div', {
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
                ...
                // v2.0-1: Version Gate 徽标（AutoIteration 多版任务显示 Vn/N）
                stepState && stepState.iterations > 1 && h('span', { style: { fontSize: 12, fontWeight: 700, color: '#c084fc', border: '1px solid rgba(167,139,250,.5)', borderRadius: 4, padding: '0 6px' } },
                  T.versionGateLabel + ': V' + String((stepState.currentIteration || 1)) + '/' + String(stepState.iterations)
                )
              ),
            ...
            // v2.0-1: 熔断状态卡片（status=paused 时高亮警示 + 恢复按钮）
            stepState && stepState.status === 'paused' && h('div', { style: { marginTop: 8, border: '1px solid #ef4444', borderRadius: 6, padding: 8, background: 'rgba(239,68,68,.08)' } },
              h('div', { style: { color: '#f87171', fontWeight: 700, fontSize: 12 } }, T.circuitBreakerTitle),
              h('div', { style: { ...hintStyle, marginTop: 4 } },
                (stepState.stopReason || '连续打回 ≥3 次，自动暂停') + (stepState.rejectStreak ? '（rejectStreak: ' + String(stepState.rejectStreak) + '）' : '')
              ),
              h('button', { onClick: resumeExperiment, disabled: stepBusy, style: { ...btnStyle, padding: '4px 10px', fontSize: 12, marginTop: 6 } }, T.btn.resume)
            ),
            ...
            h('div', { style: { marginTop: 8, border: '1px solid var(--dsw-alias-border-l1, #3f3f46)', borderRadius: 6, padding: 8 } },
              ...
              // v1.5.1: 实验选择下拉（context 记录列表 → 精确切换任意任务；
              // 修复"最新任务"被 slice 截断/文件名排序误判后面板看不到本次任务的问题）
              contextRecords && contextRecords.length > 0 && h('div', { style: { display: 'flex', gap: 6, marginTop: 6, alignItems: 'center' } },
                h('select', {
                  value: exprId || '',
                  onChange: (e) => { const v = e.target.value; if (v) loadStepState(v) },
                  style: { ...inputStyle, margin: 0, flex: 1, fontSize: 12, background: 'var(--dsw-alias-bg-layer-1, #18181b)', color: 'var(--dsw-alias-label-primary, #e4e4e7)' }
                },
                  h('option', { value: '' }, T.loadPlaceholder),
                  contextRecords.map((r) => h('option', { key: r.file, value: r.id || r.file }, (r.id || r.file) + ' · ' + (r.status || '') + (r.channel ? ' · ' + r.channel : '')))
                )
              ),
              ...
              // v2.2.1-2: 独立 Replay 入口（不依赖步骤卡片操作区；从当前任务选步骤离线复现）
              exprId && steps && steps.length > 0 && h('div', { style: { display: 'flex', gap: 6, marginTop: 4, alignItems: 'center' } },
                h('span', { style: { fontSize: 11, color: '#a1a1aa', whiteSpace: 'nowrap' } }, T.replayTitle),
                h('select', {
                  value: replayStepId,
                  onChange: (e) => setReplayStepId(e.target.value),
                  style: { ...inputStyle, margin: 0, flex: 1, fontSize: 12 }
                },
                  h('option', { value: '' }, T.replayPickStep),
                  steps.map((s) => h('option', { key: String(s.id), value: String(s.id) }, 'Step ' + s.id + ' · ' + (T.status[s.status] || s.status)))
                ),
                h('button', {
                  onClick: () => { if (replayStepId) runReplay(replayStepId) },
                  disabled: !replayStepId || stepBusy,
                  className: 'dwr-ghost',
                  style: { ...ghostPurple, padding: '4px 10px', fontSize: 12, opacity: (!replayStepId || stepBusy) ? 0.6 : 1 }
                }, T.btn.replayRun)
              )
            ),
            ...
            steps && steps.length > 0 && h('div', { style: { marginTop: 8, border: '1px solid var(--dsw-alias-border-l1, #3f3f46)', borderRadius: 6, padding: 8 } },
              h('div', { style: { ...hintStyle, fontWeight: 600, color: '#93c5fd' } }, T.stepListExec),
                ...
                stepState && stepState.autoReview && h('span', { style: { fontSize: 11, color: '#7c3aed', marginLeft: 6 } }, '· ' + T.autoReviewModeLabel),
                  ...
                  // v3.2.4: 审核通道选择（auto=API 优先+网页兜底；web-gemini=强制网页审核，过程可见）
                  // v4.9.2/hb_1: 新增 claude-code=强制本地 Claude Code 审核（混合架构协议 v2.0 通道，经 D:\cc-tasks 派发）
                  h('select', {
                    value: reviewChannel,
                    onChange: (e) => setReviewChannel(e.target.value),
                    title: T.reviewChannelTitle,
                    'aria-label': T.reviewChannelTitle,
                    style: { ...inputStyle, margin: '0 0 0 6px', padding: '2px 6px', fontSize: 11, width: 'auto' }
                  },
                    h('option', { value: 'auto' }, T.reviewChannelAuto),
                    h('option', { value: 'web-gemini' }, T.reviewChannelWebGemini),
                    h('option', { value: 'claude-code' }, T.reviewChannelClaudeCode)
                  ),
                  // P2(v3.5.0): Swarm 双角色盲审开关（默认开，importance:high 生效；title 为成本提示）
                  h('label', {
                    title: T.swarmHint,
                    style: { fontSize: 11, marginLeft: 6, color: '#a1a1aa', display: 'inline-flex', alignItems: 'center', gap: 3, cursor: 'pointer' }
                  },
                    h('input', { type: 'checkbox', checked: swarmOn, onChange: (e) => setSwarmOn(e.target.checked), style: { margin: 0, cursor: 'pointer' } }),
                    T.swarmLabel
                  ),
                  // v3.6.0 Step3: 影子沙盒门禁全局开关（默认启用；勾选=Disable）
                  h('label', {
                    title: T.shadowOffHint,
                    style: { fontSize: 11, marginLeft: 6, color: '#a1a1aa', display: 'inline-flex', alignItems: 'center', gap: 3, cursor: 'pointer' }
                  },
                    h('input', { type: 'checkbox', checked: shadowOff, onChange: (e) => setShadowOff(e.target.checked), style: { margin: 0, cursor: 'pointer' } }),
                    T.shadowOffLabel
                  ),
              ...
              // v2.1-2: DAG 拓扑图预览（分层 flex + 层间箭头；点击节点展开对应步骤明细）
              dagPreviewLayers.length > 1 && h('div', { style: { marginTop: 8, border: '1px solid rgba(96,165,250,.25)', borderRadius: 6, padding: 6, background: 'rgba(96,165,250,.04)' } },
                h('div', { style: { fontSize: 11, color: '#93c5fd', fontWeight: 600, marginBottom: 4 } }, T.dagPreviewTitle),
                dagPreviewLayers.map((layer, li) =>
                  h('div', { key: 'dagl' + li },
                    h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' } },
                      layer.map((s) => h('span', {
                        key: String(s.id),
                        // v2.1.1: low 步骤切换展开明细；所有步骤定位高亮列表项 2.5s
                        onClick: () => {
                          if (s.importance === 'low') setLowExpanded((prev) => ({ ...prev, [String(s.id)]: !prev[String(s.id)] }))
                          setDagFocus(String(s.id))
                          setTimeout(() => setDagFocus((f) => (f === String(s.id) ? null : f)), 2500)
                        },
                        title: s.title,
                        style: {
                          fontSize: 11, cursor: 'pointer', padding: '2px 8px', borderRadius: 4, whiteSpace: 'nowrap',
                          color: 'var(--dsw-alias-label-primary, #e4e4e7)',
                          border: '1px solid ' + colorOfStatus(s.status),
                          background: String(s.id) === dagFocus ? 'rgba(96,165,250,.25)' : 'rgba(24,24,27,.6)'
                        }
                      },
                        h('span', { style: { display: 'inline-block', width: 8, height: 8, borderRadius: 4, marginRight: 6, background: colorOfStatus(s.status) } }),
                        String(s.id) + ' · ' + (s.title || '').slice(0, 12)
                      ))
                    ),
                    li < dagPreviewLayers.length - 1 && h('div', { style: { textAlign: 'center', color: '#3f3f46', fontSize: 12, margin: '2px 0' } }, '↓')
                  )
                )
              ),
              ...
              steps.map((s) => {
                ...
                return h('div', { key: s.id, style: { marginTop: 6, padding: 6, background: 'var(--dsw-alias-bg-layer-1, #18181b)', border: '1px solid ' + (String(s.id) === dagFocus ? '#60a5fa' : 'var(--dsw-alias-border-l1, #3f3f46)'), borderRadius: 6, boxShadow: String(s.id) === dagFocus ? '0 0 0 1px rgba(96,165,250,.4)' : 'none', transition: 'border-color .2s, box-shadow .2s' } },
                  h('div', { style: { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' } },
                    ...
                    s.reviewedBy && h('span', {
                      style: {
                        fontSize: 10, fontWeight: 600, marginLeft: 6, padding: '1px 6px', borderRadius: 4, whiteSpace: 'nowrap',
                        color: s.reviewedBy === 'external' ? '#93c5fd' : s.reviewedBy === 'dialog' ? '#a78bfa' : s.reviewedBy === 'mainagent' ? '#4ade80' : s.reviewedBy === 'claude-code' ? '#f97316' : '#fbbf24',
                        border: '1px solid ' + (s.reviewedBy === 'external' ? 'rgba(147,197,253,.5)' : s.reviewedBy === 'dialog' ? 'rgba(167,139,250,.5)' : s.reviewedBy === 'mainagent' ? 'rgba(74,222,128,.5)' : s.reviewedBy === 'claude-code' ? 'rgba(249,115,22,.5)' : 'rgba(251,191,36,.5)')
                      }
                    }, s.reviewedBy === 'external' ? T.reviewerExternal : s.reviewedBy === 'dialog' ? T.reviewerDialog : s.reviewedBy === 'mainagent' ? T.reviewerMainagent : s.reviewedBy === 'claude-code' ? T.reviewerClaudeCode : T.reviewerManual),
                  ...
                  (!lowStep || lowOpen) && h('div', {},
                    ...
                    h('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 } },
                      ...
                      // v2.5-2 (UI): Trace Replay 离线复现（rejected 步骤排查）
                      h('button', { onClick: () => runReplay(s.id), disabled: stepBusy, className: 'dwr-ghost', style: { ...ghostPurple, padding: '2px 8px', fontSize: 11 } }, (stepBusy && stepBusyAction === 'replay') ? 'Replay…' : '🔁 Replay')
                    )
                  )
                )
              }),
              // v2.5-2 (UI): Trace Replay 还原结果展示（timeline / review / relatedTrace）
              replayResult && h('div', { style: { marginTop: 8, border: '1px solid rgba(96,165,250,.35)', borderRadius: 6, padding: 8, background: 'rgba(96,165,250,.05)' } },
                h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
                  h('div', { style: { fontSize: 12, fontWeight: 600, color: '#93c5fd' } }, T.replayTitle + ' ' + (replayResult.stepId || '') + ' · ' + (replayResult.offline ? 'offline' : '')),
                  h('button', { onClick: () => setReplayResult(null), className: 'dwr-ghost', style: { ...btnGhostStyle, padding: '2px 8px', fontSize: 11 } }, T.btn.clear)
                ),
                replayResult.step && h('div', { style: { ...hintStyle, marginTop: 4 } },
                  (replayResult.step.title || '') + ' · ' + (T.status[replayResult.step.status] || replayResult.step.status) + (replayResult.step.reviewedBy ? ' · ' + replayResult.step.reviewedBy : '')
                ),
                replayResult.review && h('div', { style: { marginTop: 6, padding: 6, border: '1px solid ' + (replayResult.review.result === 'approved' ? 'rgba(74,222,128,.5)' : 'rgba(248,113,113,.5)'), borderRadius: 4, fontSize: 11 } },
                  h('div', { style: { color: replayResult.review.result === 'approved' ? '#4ade80' : '#f87171', fontWeight: 600 } },
                    (replayResult.review.result === 'approved' ? T.badgeDone : T.badgeRejected) + ' · ' + (replayResult.review.by || '') + ' · ' + String(replayResult.review.at || '').replace('T', ' ').slice(0, 19)
                  ),
                  replayResult.review.reason && h('div', { style: { ...hintStyle, marginTop: 2 } }, replayResult.review.reason)
                ),
                h('div', { style: { fontSize: 11, color: '#a1a1aa', fontWeight: 600, marginTop: 6 } }, T.replayTimelineLabel),
                (replayResult.timeline || []).map((t, i) => h('div', { key: 'tl' + i, style: { marginTop: 2, fontSize: 11 } },
                  h('span', { style: { color: '#60a5fa' } }, String(t.at || '').replace('T', ' ').slice(0, 19)),
                  h('span', { style: { color: '#a1a1aa', marginLeft: 6 } }, '[' + (t.by || '') + ':' + (t.action || '') + ']'),
                  t.text && h('span', { style: { color: '#d4d4d8', marginLeft: 6 } }, String(t.text).slice(0, 80))
                )),
                (replayResult.relatedTrace || []).length > 0 && h('div', { style: { fontSize: 11, color: '#a1a1aa', fontWeight: 600, marginTop: 6 } }, T.replayTraceLabel),
                (replayResult.relatedTrace || []).map((e, i) => h('div', { key: 'rt' + i, style: { marginTop: 2, fontSize: 11 } },
                  h('span', { style: { color: '#a1a1aa' } }, '[' + (e.role || '') + ']'),
                  h('span', { style: { color: '#71717a', marginLeft: 6 } }, String(e.text || '').slice(0, 100))
                ))
              ),
              ...
              // v3.7.0 P2 Step3: 回滚到上版基线；v3.8 Step4: 回滚基线与状态展示增强（消费 steps 端点回传 stepState 字段，无后端重复逻辑）
              exprId && steps && steps.length > 0 && !finalSummary &&
              h('div', { style: { marginTop: 6 } },
                // Step4: 基线 / 非 git 降级 / 回滚历史状态行
                h('div', { style: { ...hintStyle, fontSize: 11, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 4 } },
                  (stepState && stepState.rollbackDegraded)
                    ? h('span', { style: { color: '#fbbf24' } }, '⚠ ' + T.btn.rollbackDegraded + '：' + String(stepState.rollbackDegraded.reason || '').slice(0, 80))
                    : (stepState && (stepState.iterationBaseCommit || stepState.rollbackBase))
                      ? h('span', { style: { color: '#93c5fd' } }, T.btn.rollbackBaseLabel + ': ' + String(stepState.rollbackBase || stepState.iterationBaseCommit).slice(0, 12))
                      : h('span', { style: { color: '#71717a' } }, T.btn.noRollbackBase),
                  (stepState && stepState.rolledBackAt) &&
                    h('span', { style: { color: '#4ade80' } }, T.btn.rollbackHistory + ' ' + String(stepState.rollbackBase || '').slice(0, 12) + (stepState.rollbackSteps ? ' · ' + stepState.rollbackSteps + T.btn.rollbackStepsReset : '')),
                  // v4.1.0 (实验 V1): 重启续跑状态（restartCount 跨重启续接次数 + 自动续跑/熔断提示）
                  (stepState && stepState.restartCount > 0) &&
                    h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 4, color: '#c084fc', fontWeight: 600 } },
                      '↻ ' + T.resumeCountLabel + ' ' + stepState.restartCount +
                      (stepState.status !== 'paused' ? ' · ' + T.resumeActiveNote : '')),
                  (stepState && stepState.status === 'paused' && String(stepState.stopReason || '').includes('重启续跑熔断')) &&
                    h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 4, color: '#f87171', fontWeight: 600 } }, '⛔ ' + T.resumePausedNote + ' ' + (stepState.restartCount || 0))
                ),
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                  h('button', { onClick: rollbackToBase, disabled: stepBusy, className: 'dwr-ghost', style: { ...ghostPurple, padding: '4px 10px', fontSize: 12 } },
                    (stepBusy && stepBusyAction === 'rollback') ? T.btn.rollbacking : T.btn.rollback)
                )
              ),
          ...
          traces === null
            ...
            : traces.length === 0
              ...
              : sortedTraces.map((t) => h('div', { key: t.id, style: { background: 'var(--dsw-alias-bg-layer-1, #18181b)', border: '1px solid var(--dsw-alias-border-l1, #3f3f46)', borderRadius: 6, padding: 8, marginBottom: 8 } },
                  ...
                  !expandedTrace[t.id] ? null : (() => {
                    // v3.4.0 V2 Step1: AutoIteration 版本分组（迭代: Vn/N、【Vn 标记）
                    const entries = t.entries || []
                    const groups = []
                    let cur = { version: null, total: null, items: [] }
                    const pushCur = () => { if (cur.items.length) groups.push(cur) }
                    for (const e of entries) {
                      const vm = String(e.text || '').match(/(?:迭代[:：]?\s*)?V(\d+)\/(\d+)|【V(\d+)|(?:^|\s)V(\d+)\s*[-—:：]?\s*(?:版间门|完成|评审|restructure|收口)/)
                      const v = vm ? (vm[1] || vm[3] || vm[4]) : null
                      if (v && cur.version !== v) { pushCur(); cur = { version: v, total: (vm && vm[2]) || null, items: [] } }
                      cur.items.push(e)
                    }
                    pushCur()
                    // reviewedBy 来源识别（优先级：显式 reviewedBy → external/dialog/manual 强信号 → mainagent）
                    const findRb = (text) => {
                      const t2 = String(text || '')
                      const m = t2.match(/reviewedBy[:：]?\s*(\w+)/i)
                      if (m) {
                        const r = m[1].toLowerCase()
                        if (r === 'external' || r === 'dialog' || r === 'manual' || r === 'mainagent') return r
                      }
                      if (/外部 ?AI|external/i.test(t2)) return 'external'
                      if (/对话模型|dialog|兜底/i.test(t2)) return 'dialog'
                      if (/manual ?审核|手动审核|用户手动/i.test(t2)) return 'manual'
                      if (/mainagent|主 ?agent 代审|自动豁免/i.test(t2)) return 'mainagent'
                      return null
                    }
                    const rbColor = (rb) => rb === 'external' ? '#93c5fd' : rb === 'dialog' ? '#a78bfa' : rb === 'mainagent' ? '#4ade80' : rb === 'claude-code' ? '#f97316' : '#fbbf24'
                    return h('div', {}, groups.map((g, gi) => h('div', { key: 'g' + gi, style: { marginTop: 6, border: '1px dashed ' + (g.version ? 'rgba(147,197,253,.45)' : '#27272a'), borderRadius: 6, padding: '4px 8px' } },
                      h('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
                        g.version && h('span', { style: { fontSize: 11, fontWeight: 700, color: '#c084fc', border: '1px solid rgba(167,139,250,.5)', borderRadius: 4, padding: '0 6px' } }, 'V' + g.version + (g.total ? '/' + g.total : '')),
                        !g.version && h('span', { style: { fontSize: 11, fontWeight: 600, color: '#71717a' } }, '总览'),
                        h('span', { style: { fontSize: 10, color: '#52525b' } }, g.items.length + ' 条')
                      ),
                      g.items.map((e, i) => {
                        const rb = findRb(e.text)
                        return h('div', { key: i, style: { marginTop: 4, borderLeft: '2px solid ' + (ROLE_COLOR[e.role] || '#52525b'), paddingLeft: 8 } },
                          h('div', { style: { fontSize: 11, color: ROLE_COLOR[e.role] || '#a1a1aa', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' } },
                            h('span', {}, (T.roleLabel[e.role] || e.role) + ' · ' + String(e.at || '').replace('T', ' ').replace(/\.\d+Z$/, 'Z')),
                            rb && h('span', { style: { fontSize: 10, fontWeight: 700, color: rbColor(rb), border: '1px solid ' + rbColor(rb) + '66', borderRadius: 4, padding: '0 5px' } }, 'reviewedBy: ' + rb)
                          ),
    ...
    exports.apply = apply
    // ccfeat-20260922-protoversion: 挂到工厂出口，供验收探针/测试直接引用纯函数。
    exports.resolveProtocolVersion = resolveProtocolVersion
